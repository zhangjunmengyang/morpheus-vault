...
| 笔记 | 核心内容 | 面试重点 |
|------|----------|----------|
| [[Projects/MA-RLHF/lc1/lc1-02-基础数学组件手撕|基础数学组件手撕]] | MHA/MQA/GQA/LayerNorm/RMSNorm/SwiGLU/LoRA | 各 Attention 变体参数量对比 |
| [[Projects/MA-RLHF/lc2/lc2-01-Transformer-手撕实操|Transformer-手撕实操]] | Encoder-Decoder 完整实现 | 训练并行 vs 推理串行 |
| [[Projects/MA-RLHF/lc3/lc3-01-GPT2-手撕实操|GPT2-手撕实操]] | Decoder-only + GPT-1→2→3 演进 | GPT 系列关键创新递进 |
...
| 笔记 | 核心内容 | 面试重点 |
|------|----------|----------|
| [[Projects/MA-RLHF/lc10/lc10-00-FlashAttention-手撕实操|FlashAttention-手撕实操]] | Online Softmax + 分块前向 + 反向传播 | IO 复杂度从 O(N²) 降至 O(N) |
| [[Projects/MA-RLHF/lc10/lc10-04-vLLM-V0-手撕实操|vLLM-手撕实操]] | PagedAttention + Continuous Batching 整体架构 | 物理/逻辑 KV 块映射 |
| [[Projects/MA-RLHF/lc10/lc10-01-Continue-Batching-手撕实操|Continue-Batching-手撕实操]] | 动态 Batch 调度器（TokenSlot + Dynamic Scheduler） | GPU 利用率 30% → 90%+ |
...
| 笔记 | 核心内容 | 面试重点 |
|------|----------|----------|
| [[Projects/MA-RLHF/lc-comm/lc-comm-01-分布式训练通信原语-手撕实操|分布式训练通信原语-手撕实操]] | NCCL 8 大原语 + Ring-AllReduce 推导 | 各并行范式的通信选型 |
| [[Projects/MA-RLHF/xtrain/xtrain-03b-ZeRO-手撕实操|ZeRO-手撕实操]] | ZeRO-1/2/3 + 分布式 Adam | 三阶段显存节省量化对比 |
| [[Projects/MA-RLHF/xtrain/xtrain-04b-Tensor-Parallel-手撕实操|Tensor-Parallel-手撕实操]] | 列/行并行 + 序列并行 | all-reduce 通信位置选择 |
| [[Projects/MA-RLHF/xtrain/xtrain-05b-Pipeline-Parallel-手撕实操|Pipeline-Parallel-手撕实操]] | GPipe → 1F1B → 交错式 1F1B | Bubble 率公式推导 |
| [[Projects/MA-RLHF/xtrain/xtrain-07b-MoE-Context-Parallel-手撕实操|MoE-Context-Parallel-手撕实操]] | Expert 并行 + Ring Attention | MoE all-to-all vs TP all-reduce |
| [[Projects/MA-RLHF/lc9/lc9-ray-01-Ray-分布式RL训练实操|Ray-分布式RL训练实操]] | Generator-Coordinator-Trainer 三角 | 训推分离 + 异步 GRPO |
| [[Projects/MA-RLHF/lc10/lc10-ray-01-Ray-推理系统实操|Ray-推理系统实操]] | Ray Actor vLLM 封装 + 负载均衡 | Rollout 生成侧设计 |
...
| [[Projects/MA-RLHF/lc8-GRPO/lc8-01-GRPO-手撕实操|GRPO-手撕实操]] | Clip 版 + 简化版 loss + GAE 替换 | GRPO vs PPO 无 Critic |
| [[Projects/MA-RLHF/lc8-DPO/lc8-01-DPO-手撕实操|DPO-手撕实操]] | Bradley-Terry + 隐式 reward 推导 | DPO 如何绕过 RM 训练 |
| [[Projects/MA-RLHF/lc8-KTO/lc8-01-KTO-手撕实操|KTO-手撕实操]] | 前景理论 + 单样本偏好 | KTO vs DPO 数据效率 |
| [[Projects/MA-RLHF/lc8-PPO/lc8-07-PRM-O1-Search-手撕实操|PRM-O1-Search-手撕实操]] | Process Reward + Beam Search + MCTS | Test-time compute scaling |

**完整 Notebook 实现（端到端，深入理解）**

| 笔记 | 核心内容 | 面试重点 |
|------|----------|----------|
| [[Projects/MA-RLHF/lc8-DPO/lc8-02-Bradley-Terry模型实现|Bradley-Terry模型实现]] | BT 偏好建模（DPO 数学基础） | `P(y_w>y_l) = σ(r_w - r_l)` 推导 |
| [[Projects/MA-RLHF/lc8-PPO/lc8-05-LLaMA2-Reward-Model实现|LLaMA2-Reward-Model实现]] | LLaMA2 + RM Head + BT Loss 完整实现 | RM 训练 vs 直接 DPO 的工程 tradeoff |
| [[Projects/MA-RLHF/lc8-PPO/lc8-03-RLHF-PPO-完整Pytorch实现|RLHF-PPO-完整Pytorch实现]] | 四模型 56-cell 完整实现（GAE + KL 约束全链路） | PPO 与 GRPO 的 Critic 有无差异 |
| [[Projects/MA-RLHF/lc8-PPO/lc8-08-O1-PRM搜索完整实现|O1-PRM搜索完整实现]] | PRM + MCTS/Beam Search 端到端搜索 | UCT 选择公式 + 树回溯 |
| [[Projects/MA-RLHF/lc8-GRPO/lc8-02-GRPO-完整Notebook实现|GRPO-完整Notebook实现]] | 组采样 + advantage 归一化 + KL 项完整实现 | group_std 归一化 vs GAE |
| [[Projects/MA-RLHF/lc8-GRPO/lc8-04-GRPO-KL散度三种近似|GRPO-KL散度三种近似]] | k1/k2/k3 Schulman 近似实现对比 | 精度 vs 计算成本 tradeoff |
| [[Projects/MA-RLHF/lc8-DPO/lc8-03-DPO-完整Notebook实现|DPO-完整Notebook实现]] | 偏好对处理 + log-ratio loss 完整实现 | β 温度对策略偏离的控制 |
| [[Projects/MA-RLHF/lc8-KTO/lc8-02-KTO-完整Notebook实现|KTO-完整Notebook实现]] | 前景理论偏好建模，无需成对数据 | z_ref 参考期望的计算方式 |
...
| [[Projects/MA-RLHF/xtrain/xtrain-06-Context并行RingAttention手写|xtrain-lc6-Context并行]] | Ring Attention 从零实现，超长上下文训练 | Ring Attention 通信量与 GPU 数无关 |
| [[Projects/MA-RLHF/xtrain/xtrain-07b-MoE-Context-Parallel-手撕实操|xtrain-lc7-MoE专家并行]] | Expert Parallelism + AllToAll 路由从零实现 | dispatch-combine 路径的通信计算重叠 |

### 🖼️ 多模态（1 篇）

| 笔记 | 核心内容 | 面试重点 |
|------|----------|----------|
| [[Projects/MA-RLHF/lc6/CLIP-ViT-LLaVA-手撕实操|CLIP-ViT-LLaVA-手撕实操]] | ViT + CLIP InfoNCE + LLaVA 投影 | 视觉 token 如何进 LLM |
...
- [[AI/3-LLM/RL/算法/GRPO 深度理解|GRPO深度理解]] — GRPO 理论完整推导
- RLHF-DPO-2026全景 — 偏好优化算法全谱
- Transformer架构深度解析 — 架构演进理论视角
- [[AI/3-LLM/RL/算法/OAPL-Off-Policy-RL-LLM-Reasoning|OAPL]] — off-policy RL 最新进展（与 Ray 分布式训练直接相关）
