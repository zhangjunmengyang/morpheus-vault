...
brief: "覆盖对齐技术栈(RLHF/DPO/CAI)、可扩展监督、红队攻防、Agent安全(MCP漏洞)、机械可解释性(SAE/RepE)、安全评估与治理框架的全景式综述。核心洞察：2026年安全威胁从'输出有害内容'升级到'执行有害操作'，需要系统性多层防御。面试武器库级别的AI安全完整知识图谱。"
date: 2026-02-20
updated: "2026-02-22"
...
sources:
  ...
  - "Scalable Oversight (Debate) — Irving et al. arXiv:1810.08100"
  - "Weak-to-Strong Generalization — Burns et al. arXiv:2312.09390"
  - "Indirect Prompt Injection — Greshake et al. arXiv:2302.12173"
  - "Jailbreak 综述 (GCG) — Zou et al. arXiv:2307.15043"
  - "Jailbreak 分类综述 — arXiv:2305.13860"
  - "Sleeper Agents — Hubinger et al. arXiv:2401.05566"
  - "Anthropic RSP — Responsible Scaling Policy 文档"
  - "Representation Engineering — Zou et al. arXiv:2310.01405"
  - "Circuit Breakers — Zou et al. arXiv:2406.04313"
  - "SAE / Towards Monosemanticity — Bricken et al. (Anthropic 2023)"
  - "OWASP LLM Top 10 (2025 版)"
related:
  - "[[AI/Safety/对齐技术总结|对齐技术总结]]"
  - "[[AI/LLM/RL/RLHF-DPO-2026-技术全景|RLHF-DPO 技术全景]]"
...
> 来源：Greshake et al. "Not What You've Signed Up For: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection" arXiv:2302.12173
...
> 来源：Zou et al. "Universal and Transferable Adversarial Attacks on Aligned Language Models" arXiv:2307.15043
...
> 来源：[Anthropic's Responsible Scaling Policy](https://www.anthropic.com/index/anthropics-responsible-scaling-policy) — Anthropic 官方文档（2023 年发布，2025 年更新）
...
### 原始论文
- [Training Language Models to Follow Instructions with Human Feedback](https://arxiv.org/abs/2203.02155) — InstructGPT，RLHF 三阶段流程的开山之作
- [Constitutional AI: Harmlessness from AI Feedback](https://arxiv.org/abs/2212.08073) — Anthropic CAI 方法论，理解 RLAIF 的核心论文
- [Direct Preference Optimization](https://arxiv.org/abs/2305.18290) — DPO 原始论文，跳过 RM 直接对齐
- [Sleeper Agents: Training Deceptive LLMs That Persist Through Safety Training](https://arxiv.org/abs/2401.05566) — 最令人不安的安全实验，证明 RLHF 不足以消除后门
- [Universal and Transferable Adversarial Attacks on Aligned Language Models](https://arxiv.org/abs/2307.15043) — GCG 攻击，白盒 jailbreak 里程碑
- [Weak-to-Strong Generalization](https://arxiv.org/abs/2312.09390) — OpenAI 超级对齐方向的开创性实验

### 深度解读
- [Anthropic Research Blog](https://www.anthropic.com/research) — 对齐与可解释性领域最高质量的一手资料 ⭐⭐⭐⭐⭐
- [OWASP Top 10 for LLM Applications (2025)](https://owasp.org/www-project-top-10-for-large-language-model-applications/) — Agent 安全威胁分类标准 ⭐⭐⭐⭐
- [AI Safety Fundamentals Course](https://aisafetyfundamentals.com/) — 从零构建对齐研究直觉 ⭐⭐⭐⭐

### 实践资源
- [Anthropic Inspect AI](https://github.com/UKGovernmentBEIS/inspect_ai) — 模块化安全评估框架，可直接用于红队测试
- [HarmBench](https://github.com/centerforaisafety/HarmBench) — 最全面的 Jailbreak 鲁棒性评测套件
- [TRL (Transformer Reinforcement Learning)](https://github.com/huggingface/trl) — DPO/PPO 安全对齐训练的工程实现
...
### 直接可用场景
- **企业 Agent 安全审计**：按 OWASP LLM Top 10 + MCP 安全最佳实践（第5节）检查现有 Agent 系统的 tool-use 权限、输入清理、沙箱隔离
- **红队测试实施**：使用 HarmBench 自动化套件 + PAIR 方法（第4.2节）对自有模型做安全评估，结合 Rainbow Teaming 确保攻击多样性
- **安全对齐选型**：根据对齐税矩阵（第1.3节）选择 DPO vs RLHF vs CAI，小团队优先 DPO（训练简单）+ RepE（推理时干预）

### 工程实现要点
- **Alignment Tax 最小化组合拳**：DPO 做粗粒度对齐 → RepE / Circuit Breakers 做 inference-time 精调，对齐税降低 ~30%
- **MCP 安全加固**：参数化命令（永不拼接 shell）+ chroot 文件隔离 + Capability Token 一次性授权
- **Sleeper Agent 检测**：常规 RLHF 无法消除后门 → 需要 SAE 特征扫描 + 行为一致性测试（训练时 vs 部署时）

### 面试高频问法
- Q: RLHF 的核心安全局限是什么？为什么仅靠 RLHF 不足以确保超人类 AI 的对齐？
  A: Reward Hacking + Scalable Oversight 困境 + Sleeper Agent 实验证明行为级训练无法消除后门 + Goodhart's Law（详见 Q1）
- Q: Prompt Injection 的直接注入和间接注入区别？为什么间接注入在 Agent 时代更危险？
  A: 攻击面从用户输入扩展到所有外部数据源 + 从信息风险升级到操作风险 + Confused Deputy + Memory Poisoning 持久性（详见 Q2）
...
### So What？对老板意味着什么
- **Agent 安全是下一个刚需**：从 Chat Safety 到 Agent Safety 的范式跃迁意味着安全工程师的市场需求将激增。掌握 MCP 安全审计 + 多层防御架构设计是差异化竞争力
- **可解释性是对齐的终局**：Sleeper Agents 实验证明行为级安全训练（RLHF/DPO）有根本局限，只有 Mechanistic Interpretability 能"打开黑箱"检测隐藏电路。SAE + Feature Steering 可能是比 RLHF 更精准的对齐手段

### 未解问题与局限
- **对齐税能降到零吗？** 当前 RepE + Circuit Breakers 降低 ~30-50%，但理论上完美安全与完全能力是否存在不可调和的矛盾？
- **Scalable Oversight 的递归误差**：IDA/递归奖励建模中，对齐误差是否会在多轮迭代后累积成灾难？目前没有严格的理论保证
- **开放域 Agent 的安全边界**：MCP 生态 61% 的包存在漏洞，供应链安全治理远未成熟

### 脑暴：如果往下延伸
- 将 [[AI/Safety/对齐技术总结|对齐技术总结]] 中的 DPO 变体与 RepE 结合，能否设计一个"自适应对齐强度"系统——对安全敏感查询自动增强对齐，对普通查询降低对齐税？
- SAE 特征发现 + [[AI/LLM/Application/幻觉问题|幻觉问题]] 中的 SelfCheckGPT 结合：能否通过检测"幻觉特征"的激活模式实现 real-time 幻觉阻断（类似 Circuit Breakers for Hallucination）？
- 6 个月后预判：ASL-4 级别的模型评估标准将成为行业标配，Red-teaming-as-a-Service 成为独立赛道

```mermaid
flowchart TD
    subgraph AI安全多层防御架构
        L1[模型层<br/>RLHF/DPO/CAI] --> L2[输入输出层<br/>Prompt过滤 + 输出分类器]
        L2 --> L3[Tool/Agent层<br/>MCP安全 + 沙箱 + 最小权限]
        L3 --> L4[记忆/数据层<br/>Memory Guard + RAG安全]
        L4 --> L5[运维/治理层<br/>审计日志 + 红队 + 合规]
    end
    subgraph 验证体系
        V1[行为测试<br/>HarmBench/DecodingTrust] --> V2[可解释性验证<br/>SAE特征检测]
        V2 --> V3[安全论证<br/>Interpretability for Safety Cases]
    end
    L1 -.->|RepE + Circuit Breakers| V2
```
...
- [[AI/Safety/安全对齐与红队|安全对齐与红队]] — Jailbreak 攻防详解
- [[AI/Safety/Adaptive-Regularization-Safety-Degradation-Finetuning|Adaptive-Regularization]] — 2026 最新：harmful intent 在 hidden state 中线性可分（AUROC>0.9）
- [[AI/Safety/对齐技术总结|对齐技术总结]] — RLHF/DPO/宪法 AI 技术路线对比
- [[AI/LLM/RL/Theory/RLRR-Reference-Guided-Alignment-Non-Verifiable|RLRR]] — 非可验证任务的对齐新范式
