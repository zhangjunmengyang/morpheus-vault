...
- **RLHF/RL 训练后端**：[[AI/LLM/Frameworks/verl/训练后端|verl]] 和 NeMo-Aligner 直接调用 Megatron-Core 做模型并行
...
### 脑暴：如果往下延伸
- 如果把 [[AI/LLM/Infra/模型并行策略|模型并行策略]] 的 Zero Bubble PP 和 Megatron 结合，能否进一步压缩万卡训练的 bubble？
- Megatron-Core 作为"并行原语库"，能否标准化为类似 NCCL 的行业基础层？

## 相关

> 🔗 See also: [[AI/LLM/Infra/DeepSpeed|DeepSpeed]] — ZeRO 系显存优化，与 Megatron TP/PP 互补
> 🔗 See also: [[AI/LLM/Infra/FSDP|FSDP]] — PyTorch 原生竞品方案（ZeRO-3 等价）
> 🔗 See also: [[AI/LLM/Infra/模型并行策略|模型并行策略]] — DP/TP/PP/SP/CP 全景对比
