...
- **RLHF 训练**：[[AI/LLM/Frameworks/OpenRLHF/OpenRLHF|OpenRLHF]] 和 [[AI/LLM/Frameworks/TRL/TRL 概述|TRL]] 均深度集成 DeepSpeed
...
- 如果把 [[AI/LLM/Infra/FSDP|FSDP2]] 的 DTensor + torch.compile 与 DeepSpeed 的 offload 结合，是否是最佳方案？

## 相关

> 🔗 See also: [[FSDP]] — PyTorch 原生的 ZeRO-3 实现，与 DeepSpeed 直接对标
> 🔗 See also: [[Megatron-LM]] — TP/PP 并行，与 ZeRO 互补（大规模集群常混用）
> 🔗 See also: [[AI/LLM/Infra/模型并行策略|模型并行策略]] — 从 DP 到 5D 并行的全景对比
