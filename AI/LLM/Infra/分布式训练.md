...
- **RL 场景**：用 [[AI/LLM/Infra/Ray|Ray]] 编排 actor/critic/ref 模型的分布式部署，比 torchrun 灵活
...
### 脑暴：如果往下延伸
- 结合 [[AI/LLM/Infra/混合精度训练|混合精度训练]] 的 FP8，通信量可以进一步减半
- [[AI/Foundations/DL-Basics/MoE 进阶|MoE 模型]] 引入了第四种并行——Expert Parallelism，通信模式从 AllReduce 变为 All-to-All
...
### 代码手撕（理论 → 代码）
- [[AI/LLM/Infra/分布式训练通信原语-手撕实操|分布式训练通信原语-手撕实操]] ⭐⭐⭐⭐⭐ — AllReduce/AllGather/ReduceScatter/P2P 从零实现，理解 NCCL 通信原语
- [[AI/LLM/Infra/ZeRO-手撕实操|ZeRO-手撕实操]] ⭐⭐⭐⭐⭐ — ZeRO-1/2/3 三阶段状态分区完整实现
- [[AI/LLM/Infra/Tensor-Parallel-手撕实操|Tensor-Parallel-手撕实操]] ⭐⭐⭐⭐⭐ — 行/列并行、QKV 分割的矩阵分布式实现
- [[AI/LLM/Infra/Pipeline-Parallel-手撕实操|Pipeline-Parallel-手撕实操]] ⭐⭐⭐⭐⭐ — 1F1B 调度、micro-batch、bubble 分析
- [[AI/LLM/Infra/MoE-Context-Parallel-手撕实操|MoE-Context-Parallel-手撕实操]] ⭐⭐⭐⭐ — Expert Parallelism + Context Parallelism 实现
- [[AI/LLM/Infra/Ray-分布式RL训练实操|Ray-分布式RL训练实操]] ⭐⭐⭐⭐ — RL 场景下 Ray cluster 编排
- [[AI/LLM/MA-RLHF-手撕实操-系列索引|MA-RLHF 手撕实操系列索引]] — 架构/推理/Infra/RL 全链路代码总索引

---

## See Also

- [[AI/LLM/Infra/Megatron-LM|Megatron-LM]] — TP/PP 的工业级实现，本文理论的代码落地
- [[AI/LLM/Infra/DeepSpeed|DeepSpeed]] — ZeRO 系列的原始实现框架，支持 offload 等高级特性
- [[AI/LLM/Infra/FSDP|FSDP]] — PyTorch 原生 ZeRO-3 实现，与本文 ZeRO 段落互补
- [[AI/LLM/Infra/混合精度训练|混合精度训练]] — 分布式训练的精度策略配套：BF16/FP8 直接影响通信量和显存占用
- [[AI/LLM/Infra/GPU 显存计算指南|GPU 显存计算指南]] — 估算不同并行策略下每卡显存需求的实操指南
- [[AI/Foundations/DL-Basics/MoE 进阶|MoE 进阶]] — Expert Parallelism 是分布式训练的第四维度，All-to-All 通信模式
- [[AI/LLM/Infra/Ray|Ray]] — RL 训练场景的分布式编排框架
