...
- 支持32K上下文长度
- 引入[[AI/LLM/Architecture/Attention 变体综述|分组查询注意力]]（GQA）优化推理效率
- 预训练数据覆盖多语言、代码、数学等领域

### 1.2 Qwen2（2024年初）
Qwen2 在架构和训练方面进行了全面升级：
- **扩展模型规模**：从0.5B到72B的完整尺寸矩阵
- **优化注意力机制**：全面采用 [[AI/LLM/Architecture/Attention 变体综述|分组查询注意力]]（GQA），显著提升推理速度
- **激活函数升级**：使用 [[AI/Foundations/ML-Basics/SwiGLU|SwiGLU]] 替代传统 ReLU，提升表达能力
- **位置编码改进**：采用 [[AI/LLM/Architecture/Transformer 位置编码|RoPE]] 旋转位置编码，支持更长序列
...
- **指令跟随优化**：通过大规模指令微调和[[AI/LLM/RL/RLHF-DPO-2026-技术全景|强化学习人类反馈]]（RLHF）提升对话质量
- **长上下文支持**：部分模型支持高达128K的上下文长度

### 1.4 Qwen3（2025年预期）
虽然尚未正式发布，但从技术路线图来看，Qwen3 可能会：
- 进一步扩大模型规模，可能达到千亿参数级别
- 引入更先进的[[AI/LLM/Architecture/MoE 深度解析|混合专家模型]]（MoE）架构
- 集成多模态能力，支持图像、音频等模态
- 优化推理效率，支持更高效的部署
...
### 2.3 RoPE 位置编码
Qwen 系列使用 [[AI/LLM/Architecture/Transformer 位置编码|RoPE]]（Rotary Position Embedding）进行位置编码：
...
- 128K 长上下文的实际使用质量仍受 Lost in the Middle 问题影响（参见 [[AI/LLM/Architecture/长上下文处理|长上下文处理]]）
- Qwen3 是否会引入 MoE 架构？如果是，与 [[AI/LLM/Architecture/DeepSeek-R1|DeepSeek-R1]] 的 DeepSeekMoE 会有什么差异？

### 脑暴：如果往下延伸
- Qwen2.5 + [[AI/LLM/Architecture/DeepSeek-R1|DeepSeek-R1]] 的 GRPO 训练：用 R1 的 RL 方法在 Qwen2.5 上训练推理能力，事实上 R1-Distill-Qwen 系列已经验证了这条路线
- 如果把 Qwen 的多语言能力和 [[AI/LLM/Architecture/Mamba-SSM|Mamba]] 的长序列效率结合，能否做出多语言长文档理解的"最优解"？

> 🔗 See also: [[AI/LLM/Architecture/架构范式对比|架构范式对比]] — Qwen 的 Decoder-Only 架构在全景中的定位