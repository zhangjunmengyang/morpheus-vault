...
部署目标 → H100/B100 集群？
  是 → FP8 PTQ（几乎免费）
  否 → 有 GPU（A100/4090）？
    是 → AWQ INT4（兼顾速度和质量）或 GPTQ INT4
    否 → CPU/Apple Silicon？
      是 → GGUF Q4_K_M 或 Q5_K_M
      否 → 需要极致压缩？
        是 → QAT（如 AQLM、QuIP#）
        否 → AWQ INT4
```
...
**首创：** Orca（Yu et al., 2022），现已成为所有 serving 框架的标配。
...
```
2022-05  Flash Attention v1             (Dao et al.)
2022-06  Orca - Continuous Batching      (Yu et al.)
2022-10  GPTQ                           (Frantar et al.)
2023-03  GQA in LLaMA-2                 (Ainslie et al.)
2023-06  Flash Attention v2              (Dao)
2023-06  PagedAttention / vLLM          (Kwon et al.)
2023-06  AWQ                            (Lin et al.)
2023-09  Speculative Decoding           (Leviathan / Chen et al.)
2023-09  StreamingLLM                   (Xiao et al.)
2023-09  Medusa                         (Cai et al.)
2023-12  Mixtral 8x7B                   (Mistral AI)
2024-01  SGLang + RadixAttention        (Zheng et al.)
2024-05  DeepSeek-V2 + MLA              (DeepSeek)
2024-07  Flash Attention v3             (Shah et al.)
2024-07  EAGLE-2                        (Li et al.)
2024-08  Sarathi-Serve / Chunked Prefill
2024-09  DistServe / Splitwise          (Disaggregated PD)
...
# Roofline 拐点
Balance_point = Peak_FLOPS / Memory_Bandwidth

# Speculative Decoding 期望接受长度
E[accepted] = (1 - α^(γ+1)) / (1 - α)  where α = acceptance rate

# Attention 计算复杂度
Standard: O(N² × D) FLOPs, O(N²) memory
Flash:    O(N² × D) FLOPs, O(N) memory
Sparse:   O(N × W × D) FLOPs (W = window size)
```
...
- [[AI/LLM/Inference/KV Cache 优化|KV Cache 优化]] — 推理优化的核心技术
- [[AI/LLM/Inference/Speculative Decoding|Speculative Decoding]] — 推理加速的代表方法
- [[AI/LLM/Efficiency/知识蒸馏与模型压缩-2026技术全景|知识蒸馏 2026 全景]] — 推理优化的模型侧手段
