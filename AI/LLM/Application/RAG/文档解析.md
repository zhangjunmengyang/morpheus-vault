...
在 [[AI/LLM/Application/RAG/RAG 原理与架构|RAG 系统]] 中，文档解析是整个知识检索链路的起点，其质量直接影响后续的向量化和检索效果。本文深入分析现代文档解析技术栈，涵盖PDF处理、表格识别、多模态理解和OCR方案。
...
### 脑暴：如果往下延伸
- 结合 [[AI/LLM/Application/RAG/检索策略|检索策略]] 的 Semantic Chunking，在解析后按语义切分而非固定长度
- 多引擎融合（PyPDF 快速解析 + Vision LLM 兜底复杂页面）是生产环境的最优策略
- Marker 项目正在成为 PDF→Markdown 的事实标准，值得持续关注

> 🔗 See also: [[AI/LLM/Application/RAG/RAG 原理与架构]] — 文档解析在 RAG 管线中的位置
> 🔗 See also: [[AI/LLM/Application/Embedding 与向量检索]] — 解析后的 chunk 如何向量化和检索