...
- [[AI/LLM/RL/Other-Algorithms/DEEP-GRPO-Deep-Dense-Exploration-Pivot-Resampling|DEEP-GRPO（深密探索Pivot重采样）]] — 同为 Diversity 维度，方法粒度不同：DEEP-GRPO 在 trajectory 层面重采样 error-prone pivot，IntroLLM 在 token 层面用 hₜ 动态调温——前者选"在哪个节点多探索"，后者让模型自己决定"现在该高温还是低温"
- [[AI/LLM/RL/Theory/GRPO-Improvement-Panorama-2026|GRPO 七维改进全景]] ⭐ — IntroLLM 属于第七维 Diversity 层的最细粒度实现；全景框架提供定位：trajectory-level → token-level → internal-state-conditioned token-level（由粗到精）
