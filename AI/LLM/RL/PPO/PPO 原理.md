...
- [[AI/LLM/RL/Fundamentals/KL散度|KL 散度]]惩罚防止模型偏离 SFT 基线太远（reward hacking）

### PPO vs GRPO 的核心区别

| 维度 | PPO | [[AI/LLM/RL/GRPO/GRPO 深度理解\|GRPO]] |
|------|-----|------|
...
- **Generation + Training 资源分配**：verl 的 [[AI/LLM/Frameworks/verl/HybridFlow|HybridFlow]] 用 SPMD+MPMD 混合解决此问题
...
### 脑暴：如果往下延伸
- [[AI/LLM/RL/GRPO/GRPO 深度理解|GRPO]] 去掉 Critic 后用组内相对排名替代优势估计——这是否意味着 Critic 在 LLM 场景下本身就不必要？
- 如果把 PPO 的 per-token credit assignment 和 GRPO 的 group comparison 结合，能否得到更好的方法？

## 相关

> 🔗 See also: [[AI/LLM/RL/GRPO/GRPO 深度理解|GRPO 深度理解]] — 无 Critic 的 PPO 替代方案
> 🔗 See also: [[AI/LLM/RL/Fundamentals/KL散度|KL散度]] — PPO 中 KL 惩罚的数学基础
> 🔗 See also: [[AI/LLM/RL/Fundamentals/策略梯度方法|策略梯度方法]] — PPO 的理论根基

- [[AI/LLM/RL/DPO/DPO-TRL实践|DPO]] — 不需要 RL 的偏好优化方法
- [[AI/LLM/Frameworks/TRL/TRL 概述|TRL 概述]] — PPO/GRPO/DPO Trainer 实现
- [[AI/LLM/Frameworks/verl/verl 概述|verl 概述]] — 高性能 RLHF 框架
- [[AI/LLM/Frameworks/OpenRLHF/OpenRLHF|OpenRLHF]] — 支持 PPO 的开源 RLHF 框架
- [[AI/LLM/Architecture/DeepSeek-R1|DeepSeek-R1]] — 使用 GRPO 替代 PPO 的实例
...
> - [[AI/Agent/Agentic-RL/LOOP-Leave-One-Out-PPO-Long-Horizon-Agent-RL|LOOP（Apple Research）]] — Leave-One-Out PPO：去掉 Critic（单倍 LLM 显存），用同组其他 rollout 的 outcome 估计 baseline；长 horizon IDA 任务；32B 超 o1 +9pp
> - [[AI/Agent/Agentic-RL/SCoRe-Self-Correction-via-Reinforcement-Learning|SCoRe（DeepMind）]] — 双阶段 PPO 训练 multi-turn 自我纠错：Phase 1 KL 约束初始化 + Phase 2 reward bonus；PPO 用于 behavior collapse 防护
