...
文本分块（Text Chunking）是 [[RAG-2026-技术全景|RAG]] 系统中的关键预处理步骤，直接影响检索效果和生成质量。合适的分块策略需要在语义完整性、检索粒度和计算效率之间找到最佳平衡点。不同的文档类型、检索场景和下游任务需要采用不同的分块方法。
...
相关链接：[[RAG-2026-技术全景|RAG]], [[AI/6-应用/RAG/Reranker|Reranker]], [[AI/6-应用/RAG/Advanced RAG|Advanced RAG]], [[AI/6-应用/RAG/向量数据库选型|向量数据库]], [[AI/3-LLM/LLM 评测体系|LLM 评测体系]], [[AI/6-应用/RAG/文档解析|文档解析]]