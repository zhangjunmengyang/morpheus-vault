...
brief: RAG 从 Naive 到 Agentic 的四代架构演进全景，覆盖检索器设计、向量数据库选型、Chunking 策略、Reranking、高级技术（Self-RAG/Graph RAG/HyDE）、评估体系和生产落地经验。面试武器级笔记，适合 30 分钟快速过一遍。
type: survey
domain: ai/rag
tags:
  - ai/rag
...
related:
  - "[[AI/6-应用/RAG/_MOC|RAG MOC]]"
  - "[[AI/6-应用/RAG/Advanced RAG]]"
  - "[[检索策略]]"
  - "[[Embedding 与向量检索]]"
  - "[[RAG 原理与架构]]"
---
...
这个概念最早由 Meta（Facebook AI Research）在 2020 年的论文 *"Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks"*（Lewis et al., arXiv:2005.11401）中正式提出。RAG 的核心价值在于：让 LLM **不再只靠参数记忆，而是能实时访问外部知识**。
...
```mermaid
flowchart LR
    A["Indexing Module<br/><small>多种 Chunker<br/>多种 Embedder</small>"] --> B["Retrieval Module<br/><small>Dense/Sparse/<br/>Graph/Table<br/>检索器可替换</small>"]
    B --> C["Post-Process Module<br/><small>Reranker/<br/>Compressor/<br/>Filter 可替换</small>"]
    C --> D["Generation Module<br/><small>多种 LLM<br/>Prompt Template<br/>可替换</small>"]
```
...
**Agentic RAG 的典型代表：**
- **Self-RAG**（Asai et al., 2023, arXiv:2310.11511）：LLM 自己判断是否需要检索、检索结果是否相关、生成是否 faithful
...
**论文：** *"Precise Zero-Shot Dense Retrieval without Relevance Labels"*（Gao et al., 2022, arXiv:2212.10496）
...
### 原始论文
- [Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks](https://arxiv.org/abs/2005.11401) — RAG 的奠基论文，Lewis et al. 2020，必读
- [Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection](https://arxiv.org/abs/2310.11511) — Agentic RAG 的理论基础，Asai et al. 2023
- [RAPTOR: Recursive Abstractive Processing for Tree-Organized Retrieval](https://arxiv.org/abs/2401.18059) — 层次化摘要检索，Sarthi et al. 2024
- [Precise Zero-Shot Dense Retrieval without Relevance Labels (HyDE)](https://arxiv.org/abs/2212.10496) — 假设文档检索，Gao et al. 2022
- [From Local to Global: A Graph RAG Approach](https://arxiv.org/abs/2404.16130) — Microsoft GraphRAG 2024

### 深度解读
- [RAG Survey: Retrieval-Augmented Generation for Large Language Models](https://arxiv.org/abs/2312.10997) — 2023 年最全面的 RAG 综述 ⭐⭐⭐⭐⭐
- [RAGAS: Automated Evaluation of Retrieval Augmented Generation](https://arxiv.org/abs/2309.15217) — RAG 评估框架的原始论文 ⭐⭐⭐⭐

### 实践资源
- [LlamaIndex 官方文档](https://docs.llamaindex.ai/) — RAG 专用框架，最丰富的抽象层
...
### 直接可用场景
- **企业知识库问答**：最成熟的 RAG 场景。用 Hybrid Retrieval + Reranking + Citation，配合权限控制，90% 企业 Q&A 需求可覆盖
- **智能客服/FAQ**：Naive RAG 即可满足简单 FAQ；Advanced RAG 处理复杂多轮对话
- **代码库搜索与理解**：对代码仓库建向量索引，支持自然语言查找代码片段和函数（参见 [[LLM代码生成-2026技术全景|代码生成全景]]）

### 工程实现要点
- **Chunking 起步参数**：chunk_size=512 tokens, overlap=15%，用 Recursive Splitting 作为基线
- **检索管线标配**：Hybrid Search (BM25 + Dense) → RRF 融合 → Cross-Encoder Reranker → Top-5 送入 LLM
- **向量数据库选型速查**：<10M pgvector；10M-1B Qdrant/Milvus；>1B Milvus 分布式

### 面试高频问法
- Q: RAG 和 Fine-tuning 怎么选？
  A: RAG 解决知识更新与扩展，Fine-tuning 解决行为与格式定制；最佳实践是两者结合——FT 让模型理解领域，RAG 提供最新知识
- Q: 如何减少 RAG 中的幻觉？
  A: 多层防御——高质量检索（Hybrid+Reranking）+ Prompt 约束（"只基于 context 回答"）+ Self-RAG 自评估 + NLI 后验证 + Citation 展示

## 💡 启发与思考

### So What？对老板意味着什么
- RAG 是 AI 应用落地最成熟的路径之一，几乎所有"让 LLM 访问私有数据"的需求都绕不开 RAG
- 掌握 RAG 全栈（检索器 + 向量库 + Reranking + 评估）是 AI 工程师的核心竞争力
- Agentic RAG 是 2025-2026 的前沿方向，懂这个方向可以在面试中拉开差距

### 未解问题与局限
- **多模态 RAG 仍不成熟**：图文混合检索的对齐质量有限，表格/图表的结构化检索是公开难题
- **评估体系依赖 LLM-as-Judge**：RAGAS 的 Faithfulness 等指标本身依赖 LLM 评估，存在评估器偏差
- **Graph RAG 构建成本过高**：大规模语料的实体/关系抽取需要海量 LLM 调用，ROI 尚不清晰

### 脑暴：如果往下延伸
- RAG + Agent 的深度融合：Agent 不仅决定"检索什么"，还决定"用什么工具检索"——SQL、API、Web Search、Knowledge Graph 混合调度
- 结合 [[LLM代码生成-2026技术全景|代码生成]] 的 RAG over Codebase，是 Cursor/Claude Code 的核心技术栈
- 6 个月后 Agentic RAG 可能成为 LLM 框架的标准组件，就像 Reranking 在 2024 年变成标配一样

## See Also

- [[AI/6-应用/RAG/_MOC|RAG MOC]] — 检索增强生成全景索引
- [[AI/6-应用/RAG/Advanced RAG|Advanced RAG]] — 进阶检索技术详解
- [[RAG 检索策略|RAG 检索策略]] — 检索层策略详解
- [[向量数据库选型|向量数据库选型]] — 基础设施选型
- [[RAG-Anything-Multimodal-RAG-Framework|RAG-Anything]] — 多模态 RAG 前沿扩展（arXiv:2503.17347）
