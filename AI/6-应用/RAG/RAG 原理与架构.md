...
related:
  ...
  - "[[AI/6-应用/RAG/检索策略]]"
  - "[[AI/6-应用/RAG/文档解析]]"
  - "[[AI/6-应用/Embedding/Embedding 与向量检索]]"
---
...
### Q4: RAG 中的检索策略有哪些？
**A**: 主要包括：1）稠密检索：基于向量相似度（BERT、Sentence-BERT）；2）稀疏检索：基于关键词匹配（BM25、TF-IDF）；3）混合检索：结合稠密和稀疏方法；4）分阶段检索：粗排+精排；5）[[AI/6-应用/RAG/检索策略|]]：HyDE、Query Expansion、Multi-vector等高级策略。
...
### 脑暴：如果往下延伸
- 结合 [[AI/6-应用/RAG/检索策略|]] 的 HyDE/Query Decomposition 可显著提升复杂查询的效果
- Agentic RAG（参见 RAG 2026 全景）让检索从固定管线变成动态决策

> 🔗 See also: [[AI/6-应用/Embedding/Embedding 与向量检索]] — Embedding 选型和向量数据库是 RAG 检索质量的基座
