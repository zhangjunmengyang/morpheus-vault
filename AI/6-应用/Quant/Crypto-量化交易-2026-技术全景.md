...
- [[Agentic-RL-2026前沿综合分析|Agentic RL 2026综合分析]] — §8.5 AI Agent Trading 的理论基础：Agent RL 的四大维度框架（探索/信用/稳定/泛化）同样适用于量化 Agent 设计
- [[TTC-Test-Time-Compute-Efficiency-2026-综合分析|TTC 效率 2026综合分析]] — §2.5 LLM情绪分析的计算成本视角：实时情绪分析的token效率是 latency-sensitive 量化策略的关键约束
- [[AI Agent 集体行为与安全漂移|AI Agent 集体行为与安全漂移]] — §2.6.194行"Multi-Agent 竞争：RL agent 博弈均衡漂移"的安全视角——多个量化 Agent 共存会产生集体行为涌现，与安全漂移机制同构
- [[MARS-Margin-Aware-Reward-Modeling-Self-Refinement|MARS]] — Reward Model 设计视角：§8.5 AI Agent Trading 中 Sharpe Ratio 作为 reward 的方法论，与 MARS 的 margin-aware reward 建模思路有方法论共鸣
- [[机器学习|机器学习]] — §3 AI/ML 技术（TFT/N-BEATS/CNN-LSTM）的基础理论支撑
