...
rating: "★★★☆☆"
tags: [agentic-RL, uncertainty, reward-shaping, failure-trajectory, credit-assignment, exploration]
related:
  - "[[AI/Agent/Agentic-RL/CSO-Verified-Critical-Step-Optimization|CSO — 反事实验证失败轨迹]]"
  - "[[AI/Agent/Agentic-RL/GiGPO-Group-in-Group-Policy-Optimization|GiGPO — step-level credit]]"
  - "[[AI/Agent/Agentic-RL/ERL-Experiential-Reinforcement-Learning|ERL — 反思内化失败经验]]"
...
**失败轨迹利用谱系（三种深度）：**
- [[AI/Agent/Agentic-RL/CSO-Verified-Critical-Step-Optimization|CSO（arXiv:2602.03412）]] — 失败轨迹 + 反事实验证（PRM + expert 生成 + rollout 验证），信号最可靠但成本最高（需 expert model）；SELAUR 是其零成本替代方案
- [[AI/Agent/Agentic-RL/ERL-Experiential-Reinforcement-Learning|ERL（arXiv:2602.13949）]] — 失败轨迹 + 反思循环（生成反思 Δ → 重试 → SFT 蒸馏），中等成本；SELAUR 更轻量但信息深度不及 ERL

**Credit Assignment 协同（成功 vs 失败双维度）：**
- [[AI/Agent/Agentic-RL/GiGPO-Group-in-Group-Policy-Optimization|GiGPO（arXiv:2505.10978）]] — step-level credit assignment 处理**成功**轨迹；与 SELAUR 正交互补：GiGPO 精化成功信号，SELAUR 激活失败信号；两者组合是 multi-turn RL 训练信号的完整覆盖
