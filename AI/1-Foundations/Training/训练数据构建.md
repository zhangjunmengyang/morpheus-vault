---
brief: "训练数据构建（面试版）——SFT 数据的质量标准（多样性/准确性/格式规范）；数据配比的经验原则（领域 vs 通用 = 1:5）；数据去重/毒性过滤/难样本挖掘的工程 SOP；面试被问如何构建训练数据的标准框架。"
title: "LLM 训练数据构建"
date: 2026-02-14
...
> [!info] 📖 版本说明
> 本篇为**面试速查版**（简洁直接）。深度工程版：[[AI/3-LLM/SFT/训练数据构建|训练数据构建工程深度版]]
...
- [[AI/3-LLM/SFT/训练数据构建|训练数据构建（SFT 深度版）]] — 本文面试版，深度版含 ChatML 格式 + quality filtering 代码 + 生产数据 pipeline
- [[合成数据与数据飞轮-2026技术全景|合成数据与数据飞轮 2026 全景]] ⭐ — 当实际数据不够时，合成数据是训练数据构建的核心扩展路径；Self-Instruct/RLAIF/数据飞轮
- [[LLM-数据工程-2026-技术全景|LLM 数据工程 2026 全景]] — 预训练数据工程全景（3778行）；本文聚焦 SFT 数据，全景版覆盖预训练→SFT完整链路
- [[预训练流程|预训练流程]] — 数据构建是预训练流程的第一步；FineWeb 的质量过滤方法与 SFT 数据过滤共享 deduplication + quality signal 设计模式
