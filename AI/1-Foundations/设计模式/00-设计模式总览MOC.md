...
├── 【第六层】架构风格与演化模式          系统的宏观形状
│     风格：分层 / 六边形 / 事件驱动 / 微核 / DDD
│     演化：绞杀者 / 特性开关 / 蓝绿部署 / 金丝雀发布
│     原则：SOLID / DRY / YAGNI / 关注点分离
│
└── 【第七层】AI 工程实践（LLMOps）      生产环境存活能力
      Prompt：Few-Shot / CoT / 版本管理 / 结构化输出
      RAG：高级RAG / 分块策略 / 评测（RAGAS）
      评测：LLM-as-Judge / 在线离线 / 回归测试套件
      护栏：输入护栏 / 输出护栏 / Prompt Injection 防护
      运维：全链路追踪 / 成本监控 / 降级策略
```
