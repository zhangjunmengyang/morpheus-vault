...
flowchart TD
  n1["用户请求"]
  n2["↓"]
  n3["[规划器 Planner]  →  任务分解树"]
  n4["├─ 子任务1"]
  n5["├─ 子任务2"]
  n6["└─ 子任务3"]
  n7["↓"]
  n8["[执行器 Executor]  →  逐步执行各子任务"]
  n9["↓"]
  n10["[重规划]  →  根据执行结果调整计划（可选）"]
  n11["↓"]
  n12["最终结果"]
  n1 --> n2
  n2 --> n3
...
````mermaid
flowchart TD
  n1["ReAct：        Think → Act → Observe → Think → Act → ...（交替进行）"]
  n2["Plan-Execute： Plan（一次） → Execute → Execute → Execute（计划先行）"]
  n1 --> n2
````
...
flowchart TD
  n1["Agent"]
  n2["├─ 工作记忆（Context Window）：当前对话，短暂，有限"]
  n3["├─ 情节记忆（向量库）：过去经历，语义检索，RAG"]
  n4["├─ 语义记忆（知识图谱/文档库）：结构化知识"]
  n5["└─ 程序记忆（技能库）：可复用的操作序列/工具"]
  n6["每次推理前：检索相关记忆 → 注入上下文"]
  n7["每次推理后：写入新记忆"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
flowchart TD
  ...
  n3["成功？"]
  n4["├─ 是  →  返回结果"]
  n5["└─ 否  →  错误信息 + 修复提示  →  LLM 重新生成"]
  n6["↑__________________________|"]
  n7["（最多重试 N 次）"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
flowchart TD
  ...
  n2["对话 Agent"]
  n3["├─ 意图识别：需要订票 + 订酒店"]
  n4["├─ 移交 → 机票 Agent（携带上下文）"]
  n5["└─ 移交 → 酒店 Agent（携带上下文）"]
  n6["关键：移交时必须携带完整上下文（否则用户要重新解释）"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
  n4 --> n5
...
flowchart TD
  ...
  n4["└─  Agent C → 答案C + 论据"]
  n5["↓"]
  n6["[辩论轮次]：各 Agent 看到对方答案，提出质疑"]
  n7["↓"]
  n8["[裁判 Agent 或投票]：综合得出最终答案"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
  n4 --> n5
...
flowchart TD
  ...
  n6["└─ Model C（擅长创意）→ 答案C"]
  n7["↓"]
  n8["[聚合器 LLM]  →  综合三个答案  →  最终答案"]
  n9["TogetherAI 的 MoA 实验：GPT-4 级别的质量，用小模型混合实现"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
  n4 --> n5
...
flowchart TD
  ...
  n4["Agent C：我觉得答案是 Y"]
  n5["→ 多数投票 → X 胜出"]
  n6["选举（Leader Election）："]
  n7["某个 Agent 挂了，触发选举"]
  n8["→ Raft / Paxos 算法 → 新 Leader 产生"]
  n9["→ 新 Leader 接管协调工作"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
...
````mermaid
flowchart TD
  n1["传统软件设计模式（1994, GoF）"]
  n2["↓"]
  n3["分布式系统模式（2000s, 微服务时代）"]
  n4["↓"]
  n5["AI Agent 模式（2023-2026）"]
  n6["↓"]
  n7["下一步（推测）："]
  n8["- 自适应模式：Agent 自主发现并采用最优协作模式"]
  n9["- 进化模式：系统通过 RL 学习新的协作结构"]
  n10["- 具身模式：物理世界中 Agent 的协作模式"]
  n1 --> n2
  n2 --> n3
  n3 --> n4
  n4 --> n5
...
flowchart TD
  n1["单 Agent 内部"]
  n2["┌─────────────────────────────────────────────────────┐"]
  n3["│  输入 → [记忆检索] → [规划] → [工具调用] → [输出]   │"]
  n4["│                         ↑                           │"]
  n5["│              [反思/验证-修复 循环]                   │"]
  n6["└─────────────────────────────────────────────────────┘"]
  n7["多 Agent 协作（选其一或组合）"]
  n8["┌──────────────┐  ┌──────────────┐  ┌──────────────┐"]
  n9["│  管道         │  │   黑板        │  │  监督者       │"]
  n10["│ A→B→C→D      │  │  共享工作区   │  │  中心管控     │"]
  n11["└──────────────┘  └──────────────┘  └──────────────┘"]
  n12["┌──────────────┐  ┌──────────────┐  ┌──────────────┐"]
  n13["│  蜂群         │  │  辩论         │  │  分层规划     │"]
  n14["│  去中心并行   │  │  对抗验证     │  │  递归分解     │"]
...
flowchart TD
  ...
  n13["│    各 Agent"]
  n14["│         │"]
  n15["│    [工具增强] ← MCP / Function Calling"]
  n16["│    [记���增强] ← RAG / Vector DB"]
  n17["│    [反思循环] ← 自我批评"]
  n18["│    [验证修复] ← 可执行验证"]
  n19["│"]
  n20["├──[熔断器/重试/超时]── 弹性保障"]
