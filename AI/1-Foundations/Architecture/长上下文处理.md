---
brief: "LLM 长上下文处理技术——RoPE 外推（YaRN/LongRoPE）/滑动窗口 Attention/StreamingLLM/KV Cache 压缩的完整方案对比；128K→1M context 的工程路径；面试被问如何支持长文档处理的标准答案框架。"
title: "LLM 长上下文处理技术"
date: 2026-02-14
...
> [!info] 📖 版本说明
> 本篇为**面试速查版**（简洁直接）。深度工程版：[[AI/3-LLM/Architecture/长上下文处理|长上下文处理工程深度版]]
...
- [[长上下文技术|长上下文技术（LLM 深度版）]] — 本文面试版，深度版含 RoPE 外推数学推导 + YaRN/NTK-Aware 完整比较 + Ring Attention 实现
- [[Transformer 位置编码|Transformer 位置编码]] — 长上下文问题的核心：RoPE 外推困难源于位置编码的训练分布限制
- [[AI/1-Foundations/Inference/KV Cache|KV Cache]] — 长上下文的工程瓶颈：128K 上下文 KV Cache 显存占用是推理部署的核心挑战，与本文的 position encoding 问题形成"理论限制+工程限制"双重约束
- [[AI/6-应用/RAG/Advanced RAG|Advanced RAG]] — 长上下文 vs RAG 的战略选择：本文面试高频问题（128K 上下文还是 RAG？）需要结合具体场景；"Lost in the Middle"问题使 RAG+精确 rerank 在某些场景仍更优
