---
brief: "LLM 采样策略——Greedy/Beam Search/Temperature/Top-K/Top-P/Min-P 的原理和适用场景；采样与温度如何影响输出多样性 vs 准确性；RepetitionPenalty/FrequencyPenalty 的工程细节；推理参数调优的完整参考。"
title: "LLM 采样策略详解"
date: 2026-02-14
tags:
  - inference
...
> [!info] 📖 版本说明
> 本篇为**面试速查版**（简洁直接）。深度工程版：[[AI/3-LLM/Inference/采样策略|采样策略工程深度版]]
...
- [[AI/3-LLM/Inference/采样策略|采样策略（LLM 深度版）]] — 本文面试版，深度版含 Min-P/典型采样/Contrastive Search 前沿方案
- [[AI/1-Foundations/Inference/Speculative Decoding|Speculative Decoding]] — 采样策略的工程延伸：Speculative Decoding 的 token 验收用"修正采样"保证分布等价性
- [[RLRR-Reference-Guided-Alignment-Non-Verifiable|RLRR（参考引导对齐）]] — 采样策略与对齐的交叉：temperature/top-p 控制 diversity，RLRR 在 non-verifiable 任务上用参考引导取代 reward，两者共同决定输出分布
- [[AI/3-LLM/Inference/LLM推理优化2026全景|LLM 推理优化 2026 全景]] ⭐ — 推理优化全景；采样策略是推理质量控制的最后一环
