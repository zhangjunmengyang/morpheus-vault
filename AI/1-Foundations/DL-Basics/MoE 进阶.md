...
### 脑暴：如果往下延伸
- MoE + [[Multi-Head Latent Attention|MLA]]：DeepSeek-V2/V3 已经证明两者可以协同——MoE 优化 FFN，MLA 优化 Attention
...
- [[MoE 基础|MoE 基础]] — 本文进阶版的前置：Expert 路由 / Top-K 门控 / 负载均衡基础
- [[MoE 深度解析|MoE 深度解析（LLM 面试版）]] — 生产级 MoE 实践：DeepSeek-V2/V3 的 Expert Parallelism + 专家微调 LoRA 策略 + 推理优化
- [[分布式训练|分布式训练]] — MoE 的 Expert Parallelism 是分布式训练的专项扩展；All-to-All 通信模式
- [[Attention 变体综述|Attention 变体综述]] — MoE 替换 FFN 层，Attention 变体替换 Attention 层；两类技术共同定义 Transformer 进化方向
- [[Multi-Head Latent Attention|Multi-Head Latent Attention]] — DeepSeek-V2/V3 同时采用 MoE + MLA，两者协同优化效率