...
- **[[VPPO-Visually-Perceptive-Policy-Optimization|VPPO]]**：同一问题（视觉捷径）的 token 级解法，两者互补
- **[[Tree-GRPO-Tree-Search-LLM-Agent-RL|Tree-GRPO（ICLR 2026）]]**：同一原则——reward 应与因果结构对齐
- **[[UI-R1-GUI-Action-Prediction-RL|UI-R1]]**：极致数据效率的另一案例（136条）
- **[[Rationale-Consistency-GenRM-Deceptive-Alignment|MetaJudge（Rationale Consistency）]]**：judging LLM 评估一致性的类似范式
...
- **对比**：[[VPPO-Visually-Perceptive-Policy-Optimization|VPPO]]（同方向，token 级视觉依赖度方案）
- **延伸**：VTPerception-R1 (2509.24776)（扩展到文本视觉感知双维度）
- **延伸**：VisRL（intention-guided focus selection 子策略，无需 region labels）
- **元原则**：[[RL-Signal-Granularity-Causal-Structure-Principle|RL 信号粒度与因果结构匹配原则]] — Perception-R1 是路径4（设计 Reward 层次）的典型实现，与 Tree-GRPO/GiGPO/VPPO 共同印证该元命题
