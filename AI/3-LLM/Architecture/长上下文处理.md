...
相关：[[AI/3-LLM/Infra/分布式训练|分布式训练]], [[AI/3-LLM/Architecture/Attention 变体综述|Attention 详解]]
...
相关：[[RAG-2026-技术全景|检索增强生成]], [[AI/3-LLM/Architecture/Attention 变体综述|注意力机制优化]]
...
相关：[[AI/1-Foundations/ML-Basics/模型评估|模型评估]], 性能优化
...
- **YaRN 是当前最务实的选择**：如果需要扩展上下文窗口，YaRN 的训练成本最低、效果最稳定，[[AI/3-LLM/Architecture/Qwen|Qwen]] 2.5 就采用了 YaRN
...
### 脑暴：如果往下延伸
- [[AI/3-LLM/Architecture/Mamba-SSM|Mamba]] 的 $O(L)$ 复杂度天然适合长序列，结合 RoPE 外推思想，能否实现真正的百万 token 高效处理？
- 如果把 Lost in the Middle 的发现反向利用——刻意在训练时对中间位置做位置增强——是否能训练出对全位置同等敏感的模型？

> 🔗 See also: [[AI/3-LLM/Architecture/架构范式对比|架构范式对比]] — 不同架构处理长序列的能力对比