---
title: Transformer 架构范式对比：Encoder-Only vs Encoder-Decoder vs Decoder-Only
brief: 全面对比 Transformer 四种架构范式（Encoder-Only/Encoder-Decoder/Decoder-Only/Prefix LM），解释 Decoder-Only 胜出的核心原因：统一的自回归目标函数、更好的 Scaling Laws 表现、In-Context Learning 涌现。包含完整的代码实现、注意力掩码可视化和面试常见问题。
type: survey
...
**统一目标函数**（Kaplan et al. *Scaling Laws for Neural Language Models* arXiv:2001.08361）：Decoder-Only 用单一的语言建模损失：
...
**Few-shot Learning**：GPT-3（arXiv:2005.14165）展示了 Decoder-Only 的涌现能力：
...
| **PaLM** | 540B | 推理能力 | 复杂推理 |
| **GPT-4**（arXiv:2303.08774） | ~1.7T | 多模态 | AGI 方向 |
...
### 原始论文
- [Attention Is All You Need](https://arxiv.org/abs/1706.03762) — Transformer 原始论文，Encoder-Decoder 架构的起点
- [BERT](https://arxiv.org/abs/1810.04805) — Encoder-Only 路线的代表
- [T5: Exploring the Limits of Transfer Learning](https://arxiv.org/abs/1910.10683) — Text-to-Text 统一框架，Encoder-Decoder 路线的巅峰
- [GPT-3: Language Models are Few-Shot Learners](https://arxiv.org/abs/2005.14165) — Decoder-Only 路线胜出的关键论文
- [Scaling Laws for Neural Language Models](https://arxiv.org/abs/2001.08361) — Decoder-Only 优势的理论解释

### 深度解读
- [What Language Model Architecture and Pretraining Objective Work Best for Zero-Shot Generalization?](https://arxiv.org/abs/2204.05832) — 系统性对比不同架构范式 ⭐⭐⭐⭐

### 实践资源
- [nanoGPT](https://github.com/karpathy/nanoGPT) — Decoder-Only 最简实现
- [T5 HuggingFace](https://huggingface.co/docs/transformers/model_doc/t5) — Encoder-Decoder 实践入口

## 🔧 落地应用

### 直接可用场景
- **架构选型决策**：通用 AI / 对话 / 推理 → Decoder-Only；纯理解任务（分类/NER）且模型 <1B → Encoder-Only；序列转换（翻译/摘要）且输入输出差异大 → Encoder-Decoder
- **面试准备**："为什么 Decoder-Only 胜出"是 LLM 方向最高频的架构面试题

### 工程实现要点
- Decoder-Only 的 **KV Cache** 只需缓存一个方向，工程实现更简单
- Encoder-Decoder 在小模型（<1B）下参数效率更高：$\sim N \times d^2$（Decoder-Only）vs $\sim 2N \times d^2$（Encoder-Decoder），但交叉注意力增加推理延迟

### 面试高频问法
- Q: Prefix LM 和 Decoder-Only 的本质区别？
  A: Prefix LM 对输入部分（prefix）使用双向注意力，生成部分使用因果注意力。本质是 Encoder-Decoder 的"软"版本——不需要显式分离 Encoder/Decoder，但保留了双向理解输入的能力。PaLM、GLM 采用此范式。

## 💡 启发与思考

### So What？对老板意味着什么
- **"一切皆生成"已成定局**：Decoder-Only 的统治意味着未来几乎所有 NLP 任务都会被转化为生成问题。理解这个趋势对选型、面试、技术判断都至关重要
- **但 Encoder 架构没死**：[[AI/3-LLM/Architecture/BERT|BERT]] 在 Embedding/Reranker/NER 等场景仍不可替代，不要因为"GPT 赢了"就忽视 Encoder 架构

### 未解问题与局限
- Decoder-Only 在纯理解任务上真的追上 Encoder 了吗？大模型下差距很小，但小模型下 BERT 仍有优势
- [[AI/3-LLM/Architecture/Mamba-SSM|Mamba]] 等 SSM 架构是否算"第五种范式"？它既不是标准 Attention，也不是 RNN

### 脑暴：如果往下延伸
- MoE + Decoder-Only（[[AI/3-LLM/Architecture/DeepSeek-R1|DeepSeek-R1]]）vs SSM + Attention 混合（Jamba）：下一个十年的架构竞争会是什么格局？
- 如果 Encoder-Decoder 在小模型场景下更高效，那 [[AI/4-模型/Qwen/Qwen3.5|Qwen]] 等开源模型是否应该为 Edge 场景提供 Encoder-Decoder 版本？

---

## See Also

- [[AI/4-模型/OpenAI/GPT|]] — Decoder-Only 路线的代表，GPT-2/3 确立自回归生成范式
- [[AI/3-LLM/Architecture/BERT|BERT]] — Encoder-Only 路线的代表，MLM 预训练奠基
- [[AI/3-LLM/Architecture/T5|T5]] — Encoder-Decoder 路线的代表，"一切任务皆 seq2seq"
- [[AI/3-LLM/Architecture/Mamba-SSM|Mamba-SSM]] — 线性复杂度 SSM，可能是第五种范式
- [[AI/3-LLM/Architecture/MoE 深度解析]] — MoE 在 Decoder-Only 框架下的扩展（DeepSeek-V3/R1）
- [[思考/Transformer架构演化的逻辑|Transformer架构演化的逻辑]] — Transformer 完整架构全景图
...
- Brown et al. *GPT-3: Language Models are Few-Shot Learners* (2020) — [arXiv:2005.14165](https://arxiv.org/abs/2005.14165)