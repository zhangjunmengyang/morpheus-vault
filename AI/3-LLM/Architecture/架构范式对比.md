...
- **但 Encoder 架构没死**：[[BERT|BERT]] 在 Embedding/Reranker/NER 等场景仍不可替代，不要因为"GPT 赢了"就忽视 Encoder 架构

### 未解问题与局限
- Decoder-Only 在纯理解任务上真的追上 Encoder 了吗？大模型下差距很小，但小模型下 BERT 仍有优势
- [[Mamba-SSM|Mamba]] 等 SSM 架构是否算"第五种范式"？它既不是标准 Attention，也不是 RNN

### 脑暴：如果往下延伸
- MoE + Decoder-Only（[[DeepSeek-R1|DeepSeek-R1]]）vs SSM + Attention 混合（Jamba）：下一个十年的架构竞争会是什么格局？
- 如果 Encoder-Decoder 在小模型场景下更高效，那 [[Qwen|Qwen]] 等开源模型是否应该为 Edge 场景提供 Encoder-Decoder 版本？

---

## See Also

- [[GPT|GPT]] — Decoder-Only 路线的代表，GPT-2/3 确立自回归生成范式
- [[BERT|BERT]] — Encoder-Only 路线的代表，MLM 预训练奠基
- [[T5|T5]] — Encoder-Decoder 路线的代表，"一切任务皆 seq2seq"
- [[Mamba-SSM|Mamba-SSM]] — 线性复杂度 SSM，可能是第五种范式
- [[MoE 深度解析]] — MoE 在 Decoder-Only 框架下的扩展（DeepSeek-V3/R1）
