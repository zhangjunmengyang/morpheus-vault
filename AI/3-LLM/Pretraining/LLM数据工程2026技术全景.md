...
1. **准确性 (Accuracy)** — 事实是否正确，数据/数字是否可验证
2. **完整性 (Completeness)** — 回答是否完整覆盖问题，是否遗漏关键信息
3. **相关性 (Relevance)** — 回答是否紧扣问题，是否有无关废话
4. **可读性 (Readability)** — 语言是否流畅自然
5. **安全性 (Safety)** — 是否有有害内容

### 5.3 数据飞轮（Data Flywheel）

```mermaid
flowchart LR
    A[用户交互] --> B[日志收集] --> C[数据清洗]
    C --> D[标注/合成] --> E[质量评估] --> F[模型训练]
    F --> G[模型部署] --> A
    G --> H[在线反馈] --> D
```
...
**自动化率：** 数据清洗 95%+、质量过滤 90%+（model-based）、标注 70-80% 合成 + 20-30% 人工验证

> **核心理念**：数据不是一次性收集，而是持续流动的生态系统——每次模型迭代产生更好的合成数据，更好的合成数据训练更好的模型，正反馈循环。
...
```mermaid
graph TD
    L1["第一层：正则匹配<br/>邮箱/电话/身份证/信用卡<br/>速度快，只能匹配已知格式"]
    --> L2["第二层：NER 模型<br/>人名/地址/组织名<br/>spaCy / Presidio / GLiNER，多语言支持"]
    --> L3["第三层：上下文感知脱敏<br/>「张三在北京工作」→ 检测人名+地点<br/>需理解语义，LLM-based"]
```
...
| 方法 | 精度 | 时间复杂度 | 适用规模 |
|------|------|-----------|---------|
| 精确去重（哈希） | 完美（完全匹配） | O(n) | 无限制 |
| MinHash LSH | 近似（可调阈值） | O(n) | 万亿 token |
| SimHash | 近似 | O(n) | 大规模，位运算快 |
| BM25/TF-IDF | 语义近似 | O(n²) | 中等规模 |
```
...
**配比原则**：没有完美公式；高质量 > 大数量；领域专用模型配比可偏向目标任务 60-80%。
...
2. **比较/排序数据**：~30-100K 条（训练 Reward Model，每条含同一 prompt 的多回答 + 人工排序）
3. **PPO prompts**：~30-50K 条多样性 prompt（不需要标注答案）

DPO 数据需求：
- **偏好对数据**：~10-100K 条 `{prompt, chosen, rejected}`，可复用 RLHF 比较数据

| 维度 | RLHF | DPO |
|------|------|-----|
| 数据复杂度 | 3 类数据 | 1 类数据 |
| 标注成本 | 高（需领域专家排序） | 中（需偏好对） |
| 训练稳定性 | 较复杂（RM + PPO） | 较简单（直接优化） |
| 适用场景 | 复杂 reward 建模 | 相对简单的偏好对齐 |
...
- [[AI/3-LLM/Pretraining/预训练数据工程|预训练数据工程]] — 理论基础版：本文工业级实践的原理对应
- [[AI/3-LLM/Pretraining/LLM预训练与分布式训练2026技术全景|LLM预训练全景]] — 训练侧配套：数据工程完成后的训练全景
