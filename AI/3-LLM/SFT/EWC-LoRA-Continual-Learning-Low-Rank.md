...
- [[AI/6-应用/OpenCharacter-Large-Scale-Synthetic-Persona-Training|OpenCharacter（合成Persona训练）]] — "LLM人格工程栈"三角：OpenCharacter用合成数据SFT建立角色风格（第1层），EWC-LoRA保证多角色持续微调时旧角色不被覆盖（第2层），PERSIST揭示两层都不足以解决结构性稳定（第3层→需要叙事锚点）
