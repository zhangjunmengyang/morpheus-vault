---
title: 文档解析：PDF/表格/多模态处理技术
brief: RAG 系统中文档解析技术栈的完整覆盖：PyPDF/PDFPlumber/Unstructured/Docling 四大 PDF 解析方案、Table Transformer/Camelot 表格识别、LayoutLM/Vision-based 多模态文档理解、PaddleOCR/Tesseract OCR 方案，含代码示例和质量评估框架。
type: tutorial
domain: ai/rag
tags:
  - ai/rag
...
related:
  - "[[AI/3-LLM/Application/RAG/RAG 原理与架构]]"
...
在 [[AI/3-LLM/Application/RAG/RAG 原理与架构|RAG 系统]] 中，文档解析是整个知识检索链路的起点，其质量直接影响后续的向量化和检索效果。本文深入分析现代文档解析技术栈，涵盖PDF处理、表格识别、多模态理解和OCR方案。

> 来源：Unstructured.io 官方文档、PyMuPDF 文档、Marker 项目（VikParuchuri/marker）、IBM Docling 项目
...
LayoutLM（Huang et al., arXiv:2204.08387）将文本、视觉、布局信息融合，实现文档理解：
...
### 工具文档
- [Unstructured.io Documentation](https://docs.unstructured.io/) — 最全面的多格式文档解析框架 ⭐⭐⭐⭐⭐
- [PyMuPDF (fitz) Documentation](https://pymupdf.readthedocs.io/) — 高性能 PDF 处理库
- [Marker: Convert PDF to Markdown](https://github.com/VikParuchuri/marker) — PDF→Markdown 转换，保留布局和表格 ⭐⭐⭐⭐

### 原始论文
- [LayoutLMv3: Pre-training for Document AI](https://arxiv.org/abs/2204.08387) — Microsoft 的多模态文档理解模型
- [Table Transformer: DETR for Table Detection](https://arxiv.org/abs/2110.00061) — Microsoft 的表格检测/结构识别

### 实践资源
- [IBM Docling](https://github.com/DS4SD/docling) — 企业级文档解析框架
- [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) — 百度开源 OCR，中文识别效果最好

## 🔧 落地应用

### 直接可用场景
- **企业文档知识库**：将内部 PDF/Word 解析后建索引，支持自然语言问答
- **金融研报解析**：PDF 中的表格、图表需要 Unstructured hi_res 或 Vision LLM 解析
- **扫描件数字化**：PaddleOCR + 版面分析，处理手写/印刷混合文档

### 工程实现要点
- **解析策略选型**：简单文本 PDF → PyPDF；含表格 → PDFPlumber/Unstructured；扫描件 → OCR；复杂布局 → Vision LLM
- **质量红线**：对重要文档（合同、报告）用 hi_res 模式 + 人工抽检；大批量普通文档用 fast 模式
- **表格处理**：Vision LLM 解析 → 转 Markdown Table → 作为独立 chunk 存储，是当前最优方案

### 面试高频问法
- Q: RAG 中文档解析的最大挑战？
  A: PDF 表格和复杂布局——OCR 类工具速度快但质量差，Vision LLM 质量好但成本高；生产中按文档重要性分级处理

## 💡 启发与思考

### So What？对老板意味着什么
- 文档解析质量是 RAG 的天花板——检索和生成再好，解析垃圾进去也是垃圾出来
- 2025-2026 Vision LLM（GPT-4o/Claude）正在颠覆传统 OCR 方案，但成本仍是瓶颈

### 未解问题与局限
- 跨页表格合并仍是难题，尤其是无边框表格
- Vision LLM 解析成本高——1000 页 PDF 用 GPT-4V 可能花费 $50+
- 手写文字 + 复杂公式的 OCR 准确率仍不够

### 脑暴：如果往下延伸
- 结合 [[AI/3-LLM/Application/RAG/检索策略|检索策略]] 的 Semantic Chunking，在解析后按语义切分而非固定长度
- 多引擎融合（PyPDF 快速解析 + Vision LLM 兜底复杂页面）是生产环境的最优策略
- Marker 项目正在成为 PDF→Markdown 的事实标准，值得持续关注

> 🔗 See also: [[AI/3-LLM/Application/RAG/RAG 原理与架构]] — 文档解析在 RAG 管线中的位置
> 🔗 See also: [[AI/3-LLM/Application/Embedding/Embedding 与向量检索]] — 解析后的 chunk 如何向量化和检索