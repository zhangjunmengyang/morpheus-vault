---
title: 混合精度训练：FP32/FP16/BF16/FP8 全景
brief: 混合精度训练通过多种数值精度平衡计算效率和数值稳定性——FP16 需要 Loss Scaling 防梯度下溢，BF16 范围等同 FP32 无需缩放成为 LLM 主流，FP8 是 H100 时代新前沿。核心参考 Micikevicius et al. arXiv:1710.03740。
type: concept
domain: ai/llm/infra
tags:
  - ai/llm/infra
...
status: active
sources:
  - Mixed Precision Training — arXiv:1710.03740 (Micikevicius et al., 2018)
  - "BFloat16: The Secret to High Performance on Cloud TPUs — Google Cloud Blog"
  - NVIDIA Transformer Engine & FP8 Training — NVIDIA Docs
  - NVIDIA Apex Mixed Precision Training — GitHub/NVIDIA/apex
related:
  - "[[AI/3-LLM/Infra/分布式训练|分布式训练]]"
  - "[[AI/3-LLM/Infra/GPU 显存计算指南|GPU 显存计算指南]]"
  - "[[AI/1-Foundations/Training/混合精度训练|混合精度训练(Foundations版)]]"
---

> [!info] 另有面试版
> Foundations 精简版：[[AI/1-Foundations/Training/混合精度训练]]
...
```mermaid
flowchart TD
    A[混合精度训练] --> B["FP32 Master Weights<br/>优化器在 FP32 上更新"]
    A --> C["Loss Scaling<br/>放大损失防梯度下溢"]
    A --> D["FP16/BF16 Forward/Backward<br/>前向和反向用低精度加速"]
...
### 脑暴：如果往下延伸
- FP8 + [[AI/3-LLM/Infra/分布式训练|分布式训练]]：通信量也可以用 FP8，AllReduce 带宽需求直接减半
- [[AI/3-LLM/Infra/GPU 显存计算指南|显存计算]]中的 $16\Phi$ 公式会随精度策略变化——纯 FP8 训练理论上可降至 $8\Phi$
...
- [[AI/3-LLM/Infra/分布式训练|分布式训练]] — 混合精度与分布式并行的组合：BF16 减少通信量，ZeRO 的优化器状态仍需 FP32
- [[AI/3-LLM/Infra/GPU 显存计算指南|GPU 显存计算指南]] — 精度选择直接影响显存公式中的字节数：FP32→BF16 参数显存减半
- [[AI/1-Foundations/Training/混合精度训练|混合精度训练(Foundations版)]] — 本文的面试精简版
- [[AI/3-LLM/Infra/DeepSpeed|DeepSpeed]] — ZeRO + 混合精度的工程整合
- [[AI/3-LLM/Architecture/Multi-Head Latent Attention|Multi-Head Latent Attention]] — MLA + FP8 KV Cache 是推理显存优化的前沿组合