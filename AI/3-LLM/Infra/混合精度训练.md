...
### 脑暴：如果往下延伸
- FP8 + [[分布式训练|分布式训练]]：通信量也可以用 FP8，AllReduce 带宽需求直接减半
- [[GPU 显存计算指南|显存计算]]中的 $16\Phi$ 公式会随精度策略变化——纯 FP8 训练理论上可降至 $8\Phi$
...
- [[分布式训练|分布式训练]] — 混合精度与分布式并行的组合：BF16 减少通信量，ZeRO 的优化器状态仍需 FP32
- [[GPU 显存计算指南|GPU 显存计算指南]] — 精度选择直接影响显存公式中的字节数：FP32→BF16 参数显存减半
- [[AI/1-Foundations/Training/混合精度训练|混合精度训练(Foundations版)]] — 本文的面试精简版
- [[DeepSpeed|DeepSpeed]] — ZeRO + 混合精度的工程整合
- [[Multi-Head Latent Attention|Multi-Head Latent Attention]] — MLA + FP8 KV Cache 是推理显存优化的前沿组合