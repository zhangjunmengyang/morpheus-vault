...
- **RL 场景**：用 [[Ray|Ray]] 编排 actor/critic/ref 模型的分布式部署，比 torchrun 灵活
...
- [[MA-RLHF-手撕实操-系列索引|MA-RLHF 手撕实操系列索引]] — 架构/推理/Infra/RL 全链路代码总索引

---

## See Also

- [[Megatron-LM|Megatron-LM]] — TP/PP 的工业级实现，本文理论的代码落地
- [[DeepSpeed|DeepSpeed]] — ZeRO 系列的原始实现框架，支持 offload 等高级特性
- [[FSDP|FSDP]] — PyTorch 原生 ZeRO-3 实现，与本文 ZeRO 段落互补
- [[AI/3-LLM/Infra/混合精度训练|混合精度训练]] — 分布式训练的精度策略配套：BF16/FP8 直接影响通信量和显存占用
- [[GPU 显存计算指南|GPU 显存计算指南]] — 估算不同并行策略下每卡显存需求的实操指南
- [[MoE 进阶|MoE 进阶]] — Expert Parallelism 是分布式训练的第四维度，All-to-All 通信模式
- [[Ray|Ray]] — RL 训练场景的分布式编排框架
- [[训练后端|verl 训练后端]] — verl 的分布式后端选择
- [[硬件资源预估|硬件资源预估]] — 资源规划
