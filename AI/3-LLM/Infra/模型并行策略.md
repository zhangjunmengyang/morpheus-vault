...
graph LR
    G0["GPU 0
Model copy
forward(batch_0)→grad_0"]
    G1["GPU 1
Model copy
forward(batch_1)→grad_1"]
    G2["GPU 2
Model copy
forward(batch_2)→grad_2"]
    G3["GPU 3
Model copy
forward(batch_3)→grad_3"]
    AR["AllReduce → 参数更新"]
    G0 --> AR
    G1 --> AR
    G2 --> AR
...
[[AI/3-LLM/Infra/Megatron-LM]] 提出——**将单个层的矩阵运算切分到多张 GPU**。
...
```mermaid
graph TD
    TOTAL["总计 16,384 GPUs
DP × PP × TP = 128 × 16 × 8"]
    DP["Data Parallel（DP = 128 组）
跨节点，每组持有完整模型"]
    PP["Pipeline Parallel（PP = 16 stages）
跨节点，按层切分流水线"]
    TP["Tensor Parallel（TP = 8）
节点内 NVLink，按 head/column 切分"]
...
- 理解 4D/5D 并行是读懂 [[AI/3-LLM/Infra/Megatron-LM]]、[[AI/3-LLM/Infra/DeepSpeed]]、[[AI/3-LLM/Infra/FSDP]] 的前提
...
### 脑暴：如果往下延伸
- 如果 [[AI/3-LLM/Architecture/FlashAttention]] 继续优化到几乎零额外显存，CP 是否还有存在必要？
- 5D 并行 + 自动搜索最优配置（如 Alpa arXiv:2201.12023）能否让用户不再手动调 TP/PP/DP？

## 相关

> 🔗 See also: [[AI/3-LLM/Infra/Megatron-LM|Megatron-LM]] — TP/PP 工业级实现
> 🔗 See also: [[AI/3-LLM/Infra/DeepSpeed|DeepSpeed]] — ZeRO 系列显存优化
> 🔗 See also: [[AI/3-LLM/Infra/FSDP|FSDP]] — PyTorch 原生 ZeRO-3

- [[AI/3-LLM/Architecture/FlashAttention]] — Attention 计算优化
- [[AI/3-LLM/Architecture/Transformer 位置编码]] — 序列并行相关
- [[AI/3-LLM/Pretraining/小规模训练手册]] — 小模型训练实践
