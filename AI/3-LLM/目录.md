...
- [[AI/3-LLM/SFT/SFT-手撕实操|SFT-手撕实操]] — Chat Template + Loss Mask + LoRA 完整代码（MA-RLHF）
...
### Slime-RL
- [[AI/3-LLM/RL/Frameworks/Slime-RL-Framework|Slime-RL]] — THUDM 异步 RL 框架
...
### 架构手撕
- [[AI/3-LLM/Architecture/基础数学组件手撕|基础数学组件]] — MHA/GQA/LayerNorm/RMSNorm/SwiGLU/LoRA
- [[AI/3-LLM/Architecture/Transformer-手撕实操|Transformer-手撕]] — Encoder-Decoder 完整实现
- [[AI/3-LLM/Architecture/GPT2-手撕实操|GPT2-手撕]] — Decoder-only + GPT-1→2→3 演进
- [[AI/3-LLM/Architecture/Llama-手撕实操|Llama-手撕]] — RoPE/GQA/KV Cache
- [[AI/3-LLM/Architecture/DeepSeek-V3-手撕实操|DeepSeek-V3-手撕]] — MLA（KV cache降16x）+ MoE
- [[AI/3-LLM/Architecture/Tokenizer-Embedding-手撕实操|Tokenizer-Embedding-手撕]] — BPE/位置编码谱系

### 推理手撕
- [[AI/3-LLM/Inference/FlashAttention-手撕实操|FlashAttention-手撕]] — Online Softmax + IO 复杂度 O(N²)→O(N)
- [[AI/3-LLM/Inference/vLLM-手撕实操|vLLM-手撕]] — PagedAttention + Continuous Batching
- [[AI/3-LLM/Inference/Continue-Batching-手撕实操|Continue-Batching-手撕]] — 动态 Batch 调度，GPU 利用率 30% → 90%+
- [[AI/3-LLM/Inference/vLLM-PageKVCache-手撕实操|vLLM-PageKVCache-手撕]] — 分页 KV Cache 管理（BlockTable + 逻辑→物理映射）
- [[AI/3-LLM/Inference/vLLM-PageAttention-手撕实操|vLLM-PageAttention-手撕]] — PagedAttention Kernel（非连续块 Attention 计算）
- [[AI/3-LLM/Inference/Chunked-Prefill-手撕实操|Chunked-Prefill-手撕]] — SARATHI：Prefill 分块 + Decode piggybacking，解决 TTFT/TPOT 矛盾
- [[AI/3-LLM/Inference/Speculative-Decoding-手撕实操|Speculative-Decoding-手撕]] — Draft-Target 对 + 拒绝采样，完整推测解码实现
- [[AI/3-LLM/Inference/PD-Disaggregation-手撕实操|PD-Disaggregation-手撕]] — Prefill-Decode 物理分离，Ray Actor + KV 异步传输
- [[AI/3-LLM/Inference/vLLM-V0-V1-完整系统实操|vLLM V0/V1 完整系统实操]] — V0 = PageKV+PageAttn 集成；V1 = SchedulerInfo + merge_prompt + KV.split 统一调度（调度粒度 request→token）

### 分布式训练手撕
- [[AI/3-LLM/Infra/分布式训练通信原语-手撕实操|通信原语-手撕]] — NCCL 8 原语 + Ring-AllReduce
- [[AI/3-LLM/Infra/ZeRO-手撕实操|ZeRO-手撕]] — ZeRO-1/2/3 显存分片
- [[AI/3-LLM/Infra/Tensor-Parallel-手撕实操|Tensor-Parallel-手撕]] — 列/行并行 + 序列并行
- [[AI/3-LLM/Infra/Pipeline-Parallel-手撕实操|Pipeline-Parallel-手撕]] — 1F1B + 交错式（Bubble率公式）
- [[AI/3-LLM/Infra/MoE-Context-Parallel-手撕实操|MoE-Context-Parallel-手撕]] — Expert并行 + Ring Attention
- [[AI/3-LLM/Infra/Ray-分布式RL训练实操|Ray-分布式RL训练-手撕]] — Generator-Coordinator-Trainer 三角架构
- [[AI/3-LLM/Infra/Ray-推理系统实操|Ray-推理系统-手撕]] — Rollout 生成侧设计

#### xtrain 从零手写（深度强化版）
- [[AI/3-LLM/Infra/xtrain-lc1-分布式通信原语从零手写|xtrain-lc1-通信原语]] — Ring-AllReduce + NCCL 8原语从零实现（不依赖框架）
- [[AI/3-LLM/Infra/xtrain-lc2-数据并行从零手写|xtrain-lc2-数据并行]] — DP/DDP 从零实现，理解 gradient sync 机制
- [[AI/3-LLM/Infra/xtrain-lc3-ZeRO优化器从零手写|xtrain-lc3-ZeRO]] — ZeRO-1/2/3 三阶段分片从零实现
- [[AI/3-LLM/Infra/xtrain-lc4-张量并行从零手写|xtrain-lc4-张量并行]] — 列/行并行 Linear + 序列并行从零实现
- [[AI/3-LLM/Infra/xtrain-lc5-流水线并行从零手写|xtrain-lc5-流水线并行]] — 1F1B + DualPipe 从零实现（Bubble率推导）
- [[AI/3-LLM/Infra/xtrain-lc6-Context并行RingAttention手写|xtrain-lc6-Context并行]] — Ring Attention 从零实现，超长上下文训练
- [[AI/3-LLM/Infra/xtrain-lc7-MoE专家并行从零手写|xtrain-lc7-MoE专家并行]] — Expert Parallelism + AllToAll 路由从零实现

### RL 对齐手撕
- [[AI/3-LLM/RL/Fundamentals/RL基础算法手撕实操|RL基础算法-手撕]] — REINFORCE/A2C/PPO/DQN
- [[AI/3-LLM/SFT/SFT-手撕实操|SFT-手撕]] — Chat Template + Loss Mask + LoRA
- [[AI/3-LLM/RL/PPO/PPO-手撕实操-MA-RLHF|PPO-手撕]] — 4模型 + GAE + MA-PPO 多适配器
- [[AI/3-LLM/RL/PPO/MA-RLHF-核心代码注解|MA-RLHF-代码注解]] — 完整项目逐行解析
- [[AI/3-LLM/RL/PPO/RLHF-PPO-完整Pytorch实现|RLHF-PPO-Notebook]] — 56-cell 完整实现（四模型架构 + GAE + KL约束全链路）
- [[AI/3-LLM/RL/PPO/LLaMA2-Reward-Model实现|LLaMA2-RM-实现]] — Bradley-Terry 偏好 RM 完整实现
- [[AI/3-LLM/RL/PPO/O1-PRM搜索完整实现|O1-PRM-搜索-Notebook]] — PRM + MCTS-O1 端到端实现（UCT选择+展开+回溯）
- [[AI/3-LLM/RL/GRPO/GRPO-手撕实操|GRPO-手撕]] — Clip版/简化版 + 与PPO差异
- [[AI/3-LLM/RL/GRPO/GRPO-完整Notebook实现|GRPO-Notebook]] — 完整端到端实现（组采样+advantage归一化+KL项）
- [[AI/3-LLM/RL/GRPO/GRPO-KL散度三种近似|GRPO-KL三种近似]] — k1/k2/k3 Schulman近似实现与对比
- [[AI/3-LLM/RL/DPO/DPO-手撕实操|DPO-手撕]] — Bradley-Terry + 隐式 reward 推导
- [[AI/3-LLM/RL/DPO/DPO-完整Notebook实现|DPO-Notebook]] — 偏好对处理 + log-ratio loss 完整实现
- [[AI/3-LLM/RL/DPO/Bradley-Terry模型实现|BT模型-实现]] — DPO 数学基础，偏好建模理论入口
- [[AI/3-LLM/RL/KTO/KTO-手撕实操|KTO-手撕]] — 前景理论 + 单样本偏好
- [[AI/3-LLM/RL/KTO/KTO-完整Notebook实现|KTO-Notebook]] — 前景理论偏好建模，无需成对数据
- [[AI/3-LLM/RL/PPO/PRM-O1-Search-手撕实操|PRM-O1-手撕]] — Process Reward + MCTS

**Batch B — 从零手写（工程级完整实现，2026-02-26）：**
- [[AI/3-LLM/MA-RLHF课程/lc8-LLaMA2-Reward-Model-从零手写|lc8 LLaMA2-RM-Batch-B]] — 生产级 RM，scalar reward head + BT Loss
- [[AI/3-LLM/MA-RLHF课程/lc8-RLHF-PPO-Pytorch从零手写|lc8 RLHF-PPO-Batch-B]] — 四模型完整 PPO，GPU 调度 + 显存管理
- [[AI/3-LLM/MA-RLHF课程/lc8-DPO-IPO-BT-偏好优化从零手写|lc8 DPO-IPO-BT-Batch-B]] — DPO/IPO/BT + GRPO-KL 三种近似
- [[AI/3-LLM/MA-RLHF课程/lc8-KTO-PRM-Search-从零手写|lc8 KTO-PRM-Batch-B]] — KTO 前景理论 + PRM-o1 搜索
- [[AI/3-LLM/MA-RLHF课程/lc8-GRPO-notebook-Pytorch从零手写|lc8 GRPO-Batch-B]] — GRPO group rollout 完整实现
...
- [[AI/3-LLM/MA-RLHF课程/lc1-基础组件-MOC|lc1 基础组件]] — BPE Tokenizer / Embedding / 位置编码全家族
- [[AI/3-LLM/MA-RLHF课程/lc2-Transformer-MOC|lc2 Transformer]] — 完整 Encoder-Decoder 实现
- [[AI/3-LLM/MA-RLHF课程/lc3-GPT系列-MOC|lc3 GPT 系列]] — GELU / PreNorm / BPE / KV Cache / ICL
- [[AI/3-LLM/MA-RLHF课程/lc4-Llama系列-MOC|lc4 Llama 系列]] — RoPE / NTK / GQA / RMSNorm / SwiGLU
- [[AI/3-LLM/MA-RLHF课程/lc5-DeepSeek-V3-MOC|lc5 DeepSeek V3]] — MoE / MLA / MTP / YaRN / mHC / Load Balance
- [[AI/3-LLM/MA-RLHF课程/lc6-SFT全链路-MOC|lc6 SFT 全链路]] — 数据处理 / LoRA / 完整训练 / RAG / ReAct / LLM-Judge
- [[AI/3-LLM/MA-RLHF课程/lc7-RL基础-MOC|lc7 RL 基础]] — MC/TD/Q-Learning/DQN/PolicyGradient/GAE
- [[AI/3-LLM/MA-RLHF课程/lc8-RL×LLM-MOC|lc8 RL×LLM]] — RLHF-PPO / DPO / KTO / GRPO / PRM / MCTS-O1
- [[AI/3-LLM/MA-RLHF课程/lc9-分布式RL训练-MOC|lc9 分布式 RL 训练]] — Ray 三角架构 / 异步 GRPO / verl 实战 / R1 复现
- [[AI/3-LLM/MA-RLHF课程/lc10-推理系统-MOC|lc10 推理系统]] — Continue Batching / PageKV / PageAttention / vLLM V0→V1 / Chunked Prefill / PD 分离 / SD
- [[AI/3-LLM/MA-RLHF课程/xtrain-分布式并行手写-MOC|xtrain 分布式并行手写]] — DP/ZeRO/TP/PP(DualPipe)/CP(RingAttn)/EP(MoE) 从零手写
