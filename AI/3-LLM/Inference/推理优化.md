...
```mermaid
graph LR
    P["Prefill（Compute-bound）
处理全部 prompt
TTFT 在这里"]
    D["Decode（Memory-bound）
逐 token 生成，每步读全量 KV
TPOT × N = 主要延迟"]
    P -->|输出 KV Cache| D
```
...
适用于 [[AI/3-LLM/Inference/Ollama|Ollama]] 本地部署。
...
• Pipeline Parallel
• Expert Parallel（MoE）"]
    CUDA["CUDA / cuBLAS / cuDNN（底层）"]

    API --> GRAPH --> QUANT --> ATTN --> PARA --> CUDA
```
...
- [[AI/3-LLM/Inference/vLLM|vLLM]] — 最流行的推理引擎
- [[AI/3-LLM/Inference/Ollama|Ollama]] — 本地部署（GGUF）
- [[AI/3-LLM/Architecture/MoE 深度解析|MoE 深度解析]] — MoE 推理特殊性
- [[AI/3-LLM/Frameworks/Unsloth/量化 & 显存预估|量化 & 显存预估]] — Unsloth 量化实践

## See Also

- [[AI/3-LLM/Inference/Thinking-by-Subtraction-Confidence-Contrastive-Decoding-Reasoning|Thinking by Subtraction]] — 低置信 token 精炼，单轨迹推理质量提升，与 MTP+ConfAdapt（高置信加速）正交互补
- [[AI/3-LLM/Inference/MTP-ConfAdapt-Self-Distillation-Inference-Speedup|MTP+ConfAdapt]] — Post-hoc 自蒸馏转 MTP，3x 解码加速，无需重训
