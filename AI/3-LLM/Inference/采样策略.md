---
title: LLM 采样策略全景：从 Greedy 到 MCTS
brief: LLM 文本生成的完整采样策略指南。覆盖基础方法（Greedy/Beam Search）、概率采样（Temperature/Top-K/Top-P/Min-P）、高级技术（Typical Sampling/Mirostat）、重复惩罚机制和任务导向配置。核心洞察：没有万能采样策略，Temperature+Top-P 组合是通用起点，不同任务需要不同参数——代码 temp=0.2、对话 temp=0.7、创意 temp=0.9。
tags:
  - ai/llm/inference
  - ai/sampling
  - ai/text-generation
...
related:
  ...
  - "[[Transformer架构深度解析-2026技术全景|Transformer 架构]]"
  - "[[AI/3-LLM/Inference/Continuous Batching|Continuous Batching]]"
  - "[[AI/3-LLM/Inference/模型部署实践|模型部署实践]]"
---

> [!info] 另有面试版
> Foundations 精简版：[[AI/1-Foundations/Inference/采样策略]]
...
```mermaid
flowchart TD
    A["任务需求"] -->|"确定性输出"| B["Greedy / Beam Search"]
    A -->|"多样性+质量"| C["概率采样"]
    B -->|"翻译/摘要"| D["Beam Search\nbeam=4, length_penalty=1.2"]
    B -->|"最快速度"| E["Greedy\ntemp=0"]
    C -->|"通用对话"| F["Top-P=0.9 + Temp=0.7"]
    C -->|"创意写作"| G["Top-P=0.95 + Temp=0.9"]
    C -->|"代码生成"| H["Top-P=0.75 + Temp=0.2"]
    C -->|"事实问答"| I["Top-P=0.85 + Temp=0.3"]
...
### 脑暴：如果往下延伸
- 如果把 [[强化学习与RLHF应用-2026全景|MCTS]] 搜索和采样策略结合，可以在推理时做 tree search + 采样的混合——对每个推理步骤做 MCTS 扩展，对最终回复做 nucleus sampling
...
> 🔗 See also: [[AI/3-LLM/Inference/KV Cache|KV Cache]] — 采样策略与 KV Cache 管理的交互（Speculative Decoding 中的 KV Cache 回滚）
> 🔗 See also: [[强化学习与RLHF应用-2026全景|RL 与 RLHF]] — MCTS + LLM 推理搜索是采样策略的高级形式
> 🔗 See also: [[AI/3-LLM/Inference/Continuous Batching|Continuous Batching]] — 采样策略影响生成长度分布，进而影响 batching 效率
> 🔗 See also: [[AI/3-LLM/Inference/模型部署实践|模型部署实践]] — 采样参数是部署配置的关键部分