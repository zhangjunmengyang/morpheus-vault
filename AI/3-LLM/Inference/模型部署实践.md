---
title: "模型部署实践"
brief: "LLM 生产部署工程实践：容器化（Docker/K8s）、推理引擎选型（vLLM/TensorRT-LLM/Triton）、GPU 集群调度、滚动更新、监控告警配置。面向工程落地，而非学术优化。"
...
## 概述
 ...
 模型部署是将训练好的大语言模型转化为生产服务的关键环节。现代模型部署需要考虑多种形态：实时 API 服务、边缘设备推理、批量处理任务等。本文深入探讨从容器化到 Kubernetes 集群管理的完整部署实践，涵盖 Triton Inference Server 的企业级应用和全方位监控策略。
...
- [[AI/3-LLM/Inference/vLLM|vLLM]] — 核心推理引擎：PagedKVCache + continuous batching 的生产实践
- [[AI/3-LLM/Inference/剪枝与蒸馏|剪枝与蒸馏]] — 部署前的模型压缩（减小部署成本）
- [[AI/3-LLM/Inference/量化综述|量化综述]] — 部署时的精度-性能权衡