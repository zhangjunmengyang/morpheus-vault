...
related:
  ...
  - "[[AI/3-LLM/RL/RARL-Reward-Modeling-Survey-论文笔记|RARL Reward Modeling Survey]]"
status: active
---
...
[[AI/3-LLM/RL/RLHF-工程全栈|RLHF 全链路]] 的标准流程：
...
[[DPO]]（Direct Preference Optimization）的核心洞察：**Reward Model 可以用 policy 本身隐式表达**。
...
[[KTO]]（Kahneman-Tversky Optimization, 2024）的突破：**不需要成对的 (chosen, rejected) 数据，只需要 pointwise 的"好/坏"标签**。
...
- **有大量偏好对数据 + 充足算力** → RLHF (PPO) 或 [[AI/3-LLM/RL/GRPO/GRPO 深度理解|GRPO]]
- **有偏好对数据 + 算力有限** → DPO（最稳妥）
- **想合并 SFT 和对齐** → ORPO
- **只有 thumbs up/down 数据** → KTO
- **需要多风格可控** → SteerLM
- **缺少人工标注** → Constitutional AI / RLAIF
- **在线 RL 探索 + 数学/代码任务** → [[AI/3-LLM/RL/GRPO/GRPO 深度理解|GRPO]] / RLOO
...
**纵深阅读（对齐技术完整图谱）：**
- [[RLHF-DPO-2026-技术全景|RLHF/DPO 2026 技术全景]] — 本综述的深度版，含 SimPO/IPO/REBEL/ReMax/REINFORCE++ 等 2025-2026 新算法全覆盖；面试终极武器
- [[AI/3-LLM/RL/RLHF-工程全栈|RLHF 全链路]] — RLHF 三阶段完整工程实操（数据/RM训练/PPO调参）

**GRPO 方向（从对齐到推理能力训练）：**
- [[AI/3-LLM/RL/GRPO/GRPO 深度理解|GRPO 深度理解]] — DeepSeek-R1 的训练算法，对齐技术向推理能力训练的演化；GRPO = RLHF 的 critic-free 简化 + group sampling

**Reward Modeling 扩展：**
- [[AI/3-LLM/RL/RARL-Reward-Modeling-Survey-论文笔记|RARL Reward Modeling Survey]] — Reward Model 的全面综述；Constitutional AI 的 RLAIF 在 RM 设计中的延伸
