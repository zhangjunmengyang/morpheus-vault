...
related:
  ...
  - "[[RLHF 全链路|RLHF 全链路]]"
  - "[[GRPO 深度理解|GRPO 深度理解]]"
  - "[[RARL-Reward-Modeling-Survey|RARL Reward Modeling Survey]]"
status: active
---
...
- **有大量偏好对数据 + 充足算力** → RLHF (PPO) 或 [[GRPO 深度理解|GRPO]]
- **有偏好对数据 + 算力有限** → DPO（最稳妥）
- **想合并 SFT 和对齐** → ORPO
- **只有 thumbs up/down 数据** → KTO
- **需要多风格可控** → SteerLM
- **缺少人工标注** → Constitutional AI / RLAIF
- **在线 RL 探索 + 数学/代码任务** → [[GRPO 深度理解|GRPO]] / RLOO
...
- [[RLHF 全链路|RLHF 全链路]] — RLHF 三阶段完整工程实操（数据/RM训练/PPO调参）

**GRPO 方向（从对齐到推理能力训练）：**
- [[GRPO 深度理解|GRPO 深度理解]] — DeepSeek-R1 的训练算法，对齐技术向推理能力训练的演化；GRPO = RLHF 的 critic-free 简化 + group sampling

**Reward Modeling 扩展：**
- [[RARL-Reward-Modeling-Survey|RARL Reward Modeling Survey]] — Reward Model 的全面综述；Constitutional AI 的 RLAIF 在 RM 设计中的延伸
