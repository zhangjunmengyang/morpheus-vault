...
related:
  - "[[AI/3-LLM/RL/算法/Dr-GRPO-Unbiased-Optimization|Dr. GRPO（原始论文，COLM 2025）]]"
  - "[[AI/3-LLM/RL/Theory/GRPO-改进七维框架分析|GRPO 改进全景]]"
  - "[[AI/3-LLM/RL/算法/SAPO-Soft-Adaptive-Policy-Optimization|SAPO（同解 advantage 问题，不同路径）]]"
  - "[[AI/3-LLM/RL/算法/OAPL-Off-Policy-RL-LLM-Reasoning|OAPL（advantage 计算修改对比）]]"
---
...
- [[AI/3-LLM/RL/算法/Dr-GRPO-Unbiased-Optimization|Dr. GRPO（原始论文，COLM 2025）]] — NoRD 所采用的核心修复方案：去掉 std 归一化消除 difficulty bias；原始论文在 LLM 数学推理域验证，NoRD 首次在自动驾驶 VLA 跨域实证其通用性
...
- [[AI/3-LLM/RL/算法/SAPO-Soft-Adaptive-Policy-Optimization|SAPO（Qwen 团队）]] — 改 clip 函数形状（hard→sigmoid 软衰减），解决 advantage 过度截断；与 Dr. GRPO 正交：SAPO 改 clip，Dr. GRPO 改 std 归一化；两者理论上可叠加
- [[AI/3-LLM/RL/算法/OAPL-Off-Policy-RL-LLM-Reasoning|OAPL（Cornell+Databricks+Harvard）]] — 从 KL-reg RL 第一性原理推导 squared loss 替代 clip；与 Dr. GRPO 正交：OAPL 改 IS-ratio 处理，Dr. GRPO 改 std 归一化
...
**VLA 与多模态 RL**
- [[AI/3-LLM/RL/算法/AT-RL-Anchor-Token-Reinforcement-Learning-Multimodal|AT-RL（视觉锚点）]] — 多模态 credit assignment，与 NoRD 的 advantage 计算修复正交；AT-RL 解决"哪些视觉 token 值得信用"，NoRD 解决"哪些样本难度应该有梯度"
...
2. [[AI/3-LLM/RL/算法/Dr-GRPO-Unbiased-Optimization|Dr. GRPO（COLM 2025）]] — difficulty bias 的理论基础
3. [[AI/3-LLM/RL/Theory/GRPO-改进七维框架分析|GRPO 改进全景]] — NoRD 在 GRPO 改进谱系中的完整定位
4. [[AI/3-LLM/RL/算法/GRPO 深度理解|GRPO 深度理解]] — 理解 std 归一化为何导致 difficulty bias 的数学基础
