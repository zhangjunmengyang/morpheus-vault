...
**verl 框架内部（同目录）**
- [[verl 概述|verl 概述]] — 框架总览，含训练后端章节
- [[HybridFlow|HybridFlow]] — 单控制器架构，训练/推理后端协同原理
- [[性能调优|性能调优]] — 后端性能调优实战
- [[硬件资源预估|硬件资源预估]] — 不同后端显存需求

**外部依赖（后端工具）**
- [[FSDP|FSDP]] — 训练后端：PyTorch 全参数分片，verl 默认训练选项
- [[Megatron-LM|Megatron-LM]] — 训练后端：Megatron 大规模分布式（3D并行）
- [[AI/3-LLM/Infra/ZeRO-手撕实操|ZeRO-手撕实操]] — FSDP 底层原理的手撕实现
- [[AI/3-LLM/Inference/vLLM-手撕实操|vLLM-手撕实操]] — 推理后端 vLLM 的手撕实现

## 推荐阅读

1. **verl 官方文档**：[verl.readthedocs.io](https://verl.readthedocs.io/) — 训练后端配置章节
2. **HybridFlow 论文**：[[HybridFlow|HybridFlow]] — 理解 verl 单控制器模型的根本原因
