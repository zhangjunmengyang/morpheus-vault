...
**关联**：[[lc8-GRPO-手撕实操]] | [[lc9-Ray分布式RL训练实操]] | [[xtrain-lc3-ZeRO优化器从零手写]]
...
- **算法基础**：[[lc8-GRPO-手撕实操]] → verl 的分布式 GRPO 就是这个算法的工业实现
- **分布式基础**：[[xtrain-lc3-ZeRO优化器从零手写]] → verl 用的 FSDP 与 ZeRO-3 原理相同
- **推理加速**：[[lc10-vLLM-PageKVCache-手撕实操]] → verl async rollout 依赖 vLLM
- **Ray 基础**：[[Ray-分布式RL训练实操]] → verl 的 RayWorkerGroup 就是这个的封装
