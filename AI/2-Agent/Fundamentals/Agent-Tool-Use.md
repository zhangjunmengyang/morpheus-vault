...
```mermaid
flowchart TD
    LLM1["LLM<br/>判断需要调用工具<br/>生成结构化调用请求"]
    --> TOOL["Tool Execution<br/>应用层执行函数<br/>输入: {name: get_weather, arguments: {city: 北京}}"]
    --> LLM2["LLM<br/>整合结果生成回复<br/>输入: {temperature: 5, condition: 晴}"]
    --> OUT["北京今天晴，气温 5°C。"]
```
...
```mermaid
graph TD
    L3["Level 3: 动态工具发现<br/>MCP / Tool Registry 按需加载"]
    --> L2["Level 2: 工具路由<br/>先分类再选工具，两阶段检索"]
    --> L1["Level 1: 全量注入<br/>所有工具 schema 放入 context"]
```
