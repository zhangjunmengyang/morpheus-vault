...
- [[Tool Use|Tool Use]] — Agent 的另一核心能力
- [[Agentic RL Training|Agentic RL Training]] — 用 RL 优化 Agent 行为
- [[Prompt-Engineering-概述|Prompt Engineering]] — system prompt 设计
- [[AWM-Agent-World-Model-Synthetic-Environments|AWM]] — 世界模型作为Agent记忆的扩展：DB-as-state是"外置记忆"的工程实现
- [[PERSIST-LLM-Personality-Stability-Benchmark|PERSIST]] — 记忆的一致性问题：记忆模块存储的信息需要与Agent人格行为保持一致（PERSIST测量正是这种跨时间稳定性）
