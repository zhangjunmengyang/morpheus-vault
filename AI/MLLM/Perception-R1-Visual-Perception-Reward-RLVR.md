...
- **对比**：[[AI/LLM/RL/Other-Algorithms/VPPO-Visually-Perceptive-Policy-Optimization|VPPO]]（同方向，token 级视觉依赖度方案）
- **延伸**：VTPerception-R1 (2509.24776)（扩展到文本视觉感知双维度）
- **延伸**：VisRL（intention-guided focus selection 子策略，无需 region labels）
- **元原则**：[[AI/LLM/RL/Theory/RL-Signal-Granularity-Causal-Structure-Principle|RL 信号粒度与因果结构匹配原则]] — Perception-R1 是路径4（设计 Reward 层次）的典型实现，与 Tree-GRPO/GiGPO/VPPO 共同印证该元命题
