---
brief: "混合精度训练——BF16/FP16/FP32 的精度特性和 LLM 训练选型；Loss Scale/Master Weight 的混合精度机制；FP8 训练的最新进展（Transformer Engine）；显存和精度的工程权衡。"
title: "混合精度训练详解"
date: 2026-02-14
...
> [!info] 📖 版本说明
> 本篇为**面试速查版**（简洁直接）。深度工程版：[[AI/LLM/Infra/混合精度训练|混合精度训练工程深度版]]
...
- [[AI/LLM/Infra/混合精度训练|混合精度训练（LLM 深度版）]] — 本文面试版，深度版含 BF16 vs FP16 工程选型 + TensorCore 利用 + 生产踩坑
- [[AI/LLM/RL/Frameworks/Jet-RL-FP8-On-Policy-RL-Training|Jet-RL（FP8 On-Policy RL）]] — FP8 是混合精度训练的前沿演化：BF16→FP8 的精度挑战与混合精度核心问题同构（数值稳定性 vs 速度）；RL 训练场景下 FP8 的特殊挑战
- [[AI/Foundations/Training/预训练流程|预训练流程]] — 混合精度训练是预训练流程的标配工程组件；loss scale + GradScaler 在 pretraining 长周期训练中的稳定性至关重要
- [[AI/LLM/Pretraining/LLM-预训练与分布式训练-2026-全景|LLM 预训练与分布式训练 2026 全景]] ⭐ — 混合精度在大规模分布式训练中的实现细节（Megatron/DeepSpeed 的精度配置）
