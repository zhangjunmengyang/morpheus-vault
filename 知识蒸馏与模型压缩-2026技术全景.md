---
title: "知识蒸馏与模型压缩 — 2026 技术全景（根目录副本）"
date: 2026-02-21
tags: [知识蒸馏, 模型压缩, 面试武器库]
domain: AI/LLM/Efficiency
status: deprecated
canonical: "[[AI/LLM/Efficiency/知识蒸馏与模型压缩-2026技术全景]]"
---
...
> [!warning] 路径偏差 — 根目录副本，请勿使用
> 此文件是 Scholar 路径错误写入的副本。正式版本位于 [[AI/LLM/Efficiency/知识蒸馏与模型压缩-2026技术全景|AI/LLM/Efficiency/]]。请使用正式版本。
...
性能（MMLU/HumanEval）
  ...
  │   ★ Claude 3.5 Sonnet
  │  ★ Llama 405B
  │ ★ DeepSeek R1 Distill 70B (蒸馏)
  │★ Qwen 2.5 72B-GPTQ-4bit (量化)
  │★ Llama 70B-pruned-50% (剪枝)
...
GPTQ 是第一个成功将 LLM 量化到 4-bit 且保持可用性的方法。
...
AWQ（Activation-Aware Weight Quantization）的核心观察是：**并非所有权重同等重要**。
...
```
GGUF 量化类型:
├── Q2_K  (2-bit, K-quant)     → ~2.6 bpw, 极端压缩
├── Q3_K_S/M/L (3-bit)        → ~3.3 bpw, 小模型勉强可用
├── Q4_K_S/M (4-bit)           → ~4.5 bpw, 质量/大小最优甜区 ★
├── Q5_K_S/M (5-bit)           → ~5.5 bpw, 接近 FP16 质量
├── Q6_K  (6-bit)              → ~6.5 bpw, 几乎无损
├── Q8_0  (8-bit)              → ~8.5 bpw, 近无损
└── IQ2_XXS/XS/S/M (imatrix) → ~2.0-2.5 bpw, importance-matrix 引导
```
...
│   ├── N:M 稀疏（如 2:4）
│   └── NVIDIA Ampere+ 硬件原生支持
│
└── 结构化剪枝（Structured）
    ├── 头剪枝（Head pruning）
    ├── 层剪枝（Layer pruning）
    ├── 通道/神经元剪枝（Channel pruning）
    └── 直接减小模型维度，无需特殊硬件
```
...
SparseGPT 是第一个能在单次前向传播中将 LLM 剪枝到 50-60% 稀疏度且几乎无损的方法。
...
Wanda（**W**eight **and** **A**ctivation pruning）更简单但出奇地有效：
...
```
原始权重 W ∈ R^{4096×4096}
  ├── SVD 分解: W ≈ A @ B (rank=2048)
  │   ├── A ∈ R^{4096×2048}, B ∈ R^{2048×4096}
  │   ├── 参数量: 从 16.8M → 16.8M（无压缩！r 太大）
  │   └── 需要更低的 rank 才有压缩效果
  │
  └── SVD(rank=1024) + INT4 量化:
      ├── A_q ∈ R^{4096×1024} (INT4) → 2.1M 参数 × 0.5B = 1.05MB
      ├── B_q ∈ R^{1024×4096} (INT4) → 2.1M 参数 × 0.5B = 1.05MB
      └── 总计 2.1 MB vs 原始 33.6 MB → 16× 压缩
```
...
用户请求
    ↓
任务路由器（端侧）
    ├── 简单任务 → 端侧模型（~3B, Neural Engine）
    │   ├── 文本摘要
    │   ├── 邮件重写
    │   ├── 智能回复建议
    │   └── 文本校对
    │
    └── 复杂任务 → Private Cloud Compute
        ├── Apple Silicon 服务器
        ├── 端到端加密
        ├── 不存储用户数据
        └── 可审计的安全架构
