...
- 主导节假日复盘、酒旅夏战等场景的 Multi-Agent 分析系统建设，落地三层架构：主 Planner（任务拆解与调度）+ 分析子 Agent（取数/代码/表格分析三工具）+ 报告生成 Agent（逐章节生成防幻觉）
- 攻克上下文崩溃难题：对内外循环分别裁剪（外循环每章清空历史、内循环过滤失败消息），彻底解决随上下文堆叠导致的分析质量崩溃；system prompt 从 3w token 压缩至 1.5w
- 训练垂类分析模型：主导 SFT + AgentRL 后训练全链路，自研模型在节假日评测集得分从 55 分提升至 74 分，逼近商业模型（QwQ-32B 76 分）；数据从人工 9 条迭代至真实 Agent 日志蒸馏，版本跨越 v0.1.0 → v0.3.0
- 建设评测自动化：设计并跑通封闭式 / 半封闭式 / 开放式主观题三类自动化方案，准确率分别达 100% / 95% / 80%+，整体评测提效约 40%
- 推动场景泛化与产品化：知识库与分析框架解耦可插拔，引入 DAG 并行调度替换串行执行，单场景报告耗时从 2h+ 显著压缩；酒旅夏战报告已投产，策略知识注入后业务洞察准确率达 80%

**Agent 架构**

- 深度参与 Agent 架构演进全过程：单 Agent → Multi-Agent（Planner/supervisor/swarm）→ 超级单 Agent，形成系统性判断：高泛化选 Agent，快落地选 Workflow，两者不互斥
...
- 设计并运行 6-Agent 长期系统（贾维斯 + 5 个专职 Agent），实验横向信息流、故障驱动进化、Agent 记忆继承等机制
- 量化结论：公告板机制将 Agent 间信息延迟从 ∞ 降至 <1h，跨 Agent 协作完成率从 12% 提升至 73%
